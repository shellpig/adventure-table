...
## KI-P1D-001 — Builder Draft 存檔／重取競態導致 E2E 不穩定

**狀態**：未處理，待 diagnose
**發現**：2026-09-04，M03-D 驗證期間跑全套 E2E；2026-09-06 P2-A 驗證期間確認影響範圍不只 P1-D
**位置**：多支 Builder E2E。已確認：

- `apps/web/e2e/character-builder.spec.ts:133`，`P1-D preserves an ordered Fighter 5 / Wizard 5 rail with ASI and feat choices`
...
兩種表現面，共同點是 Builder 對 Draft 寫入之後前端顯示沒跟上，等滿 5 秒逾時。

一、即時摘要沒刷新（`character-builder.spec.ts:133`）：
...
戰士 4 級的 ASI 兩點都投給力量後，該題本身的 `2 / 2` 斷言通過，但右側即時摘要 `.summary-abilities` 停在 STR 16（標準陣列 15 + 人類 +1），兩點 ASI 完全沒反映。不是加了一點的中間態。

二、Draft revision 沒推進（`m01e`、`m01m` 的 `waitForDraftRevision`）：
...
選項已送出，但 `.builder-save-state` 顯示的 revision 停在前一個值。

失敗的是哪一支不固定。2026-09-06 P2-A 驗證期間連跑三次全套，每次都剛好一條失敗：`character-builder` 兩次、`m01e` 一次。
...
- 人工操作重現不出來：使用者手動走完同一條流程，摘要正常顯示 18。兩次點擊之間只要有人類反應時間就不會發生，符合「連續兩次 patch 與 refetch 交錯」的競態樣態，但尚未定位到具體的 revision 比對邏輯。
- **E2E 資料殘留會加劇但不是根因**：2026-09-06 實測，`rooms` 累積到 331 筆時 `character-builder` 連三次全敗；清空殘留後回到 1 過 2 敗的偶發水準。

### 影響範圍

- 全套 E2E 幾乎每次都會有一條失敗，且失敗的 spec 不固定；`e2e-docker.mjs` 的第二輪（拿掉 `xge` 的 M03-C import 子集）會因第一輪失敗而完全不執行。
- 不影響使用者：正常速度的人工操作沒有問題。
- 受影響 spec 的其餘契約與同檔其他測試不受影響。
- **會擋住每一個 Subphase 的關門 gate**，不限單一 Phase。

### 暫時處置

**不改測試碼、不標 `test.fixme()`、不提高 timeout。**

原本的「失敗時重跑一次判定，重跑通過即註明本編號」已不適用：實測重跑只會換另一支 spec 失敗，不會整套轉綠。

現行處置為逐條核對失敗簽章：屬於本編號的是「即時摘要未刷新」或「`waitForDraftRevision` 逾時」。若該次全套 E2E 的失敗**全部**屬於本編號，視為「除本編號外通過」，據以放行該 Subphase 關門；只要出現任何其他簽章就不得放行。放行紀錄必須寫明失敗的 spec、行號與簽章。

跑全套前先讓 `e2e-global-setup.mjs` 清乾淨資料殘留（characters / drafts / rooms），可把發生機率壓回偶發水準。

### 重啟條件

進 `diagnose` 追 Builder Draft 的存檔與重取競態，兩層要一起看：

- Draft autosave：一次選項送出後，`.builder-save-state` 的 revision 為何可能不推進。
- 即時摘要：ASI patch 回應與 summary 查詢之間是否以 draft revision 排序，第二次點擊是否可能覆蓋掉尚未落地的第一次結果。

修好之前不要只是提高該處 timeout——那會把競態藏起來而不是修掉。

一併看 KI-M01J-001：它記錄的「某些控制項第一次點擊後草稿 revision 不會在逾時內遞增」很可能是同一個競態的另一個入口，修好本項後應重新評估該項是否還需要 `test.fixme()`。
