...
這是**真的沒推進**，不是等待條件寫錯。兩支受影響的 spec 都用 `expect.poll(() => currentDraftRevision(page)).toBeGreaterThan(previousRevision)`，唯一能滿足它的方式就是版號真的前進。
...
- 2026-09-07 起這個描述不再成立：修掉另一半之後全套 E2E 首次全綠（104 passed / 3 skipped / 0 failed）。本編號目前**沒有活躍的重現案例**。
