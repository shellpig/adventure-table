...
- `apps/web/e2e/m01k-phb-feats-and-spells.spec.ts:392`（經 `finishAndReview` → `fillEmptyComboboxes` → `chooseFirstEnabled` → `waitForDraftRevision`），`M01-K keeps two Elemental Adept acquisitions and a PHB spellbook spell`。2026-09-09 P3-A 關門期間新增，是本編號第一次取得**分支對照**證據：同一份 spec 以 `--repeat-each=3` 只跑本檔，在 `origin/main`（`e07aec6`，run 34295347049）與 P3-A 分支（run 34295350916）上結果完全相同——本支 3/3 失敗、卡在同一個 revision 51、同一組 stack。這確立本簽章與當時待合併的 P3-A 無關，是 main 既有問題。同期另有 main 單跑本檔一輪 6 passed（run 34294956512），說明單輪綠燈是運氣，不足以當「已修復」或「未重現」的證據。

疑似同族但證據不足：

- `apps/web/e2e/m01j-subclass-expansion.spec.ts:394`，2026-09-06 全套失敗但沒留下 error-context。
- `apps/web/e2e/m01k-phb-feats-and-spells.spec.ts:342`（`M01-K takes a PHB feat at an ASI during Level Up`）。2026-09-09 兩次全套 E2E 皆失敗，但簽章不是本編號：一次 `toHaveURL` 逾時於 spec:373（Confirm Level Up 後未導航），一次 `readSheet` 逾時於 spec:225。單跑本檔時在 main 與分支都通過；`--repeat-each=3` 時兩邊各失敗 2/3，但那批的簽章是 `toHaveCount`（spec:353）——重複執行造成同名角色出現多張 workshop card 的 harness 假象，對本支不具判別力。**缺「main 跑全套」的對照，歸屬未定。**
...
- 2026-09-07 起曾一度不成立：修掉另一半之後全套 E2E 首次全綠（104 passed / 3 skipped / 0 failed）。
- **2026-09-09 重新出現活躍重現案例。** P3-A 關門期間，全套 E2E 在 `m01k:392` 以本簽章穩定失敗（兩次全套皆失敗），且 `--repeat-each=3` 下 3/3 失敗。重現率遠高於先前的偶發水準，**「重啟條件」要求的 feedback loop 已經具備**，可直接進 diagnose。
...
**2026-09-09 起條件已滿足，可以直接開始 diagnose。** 先前擋住重啟的理由是沒有可用的 feedback loop；現在 `m01k:392` 提供了穩定重現：

```
cd apps/web
npm run test:e2e:docker -- e2e/m01k-phb-feats-and-spells.spec.ts --repeat-each=3
```

該指令在 `origin/main` 與 P3-A 分支上都是本支 3/3 失敗（run 34295347049 / 34295350916）。Windows 本機不要走 Playwright 託管 vite 的路徑，見 KI-ENV-001。

注意這個重現只保證 `:392`；同檔 `:342` 在此指令下的失敗是 `--repeat-each` 造成的同名角色假象（見上方「疑似同族但證據不足」），不要拿它當本編號的訊號。
