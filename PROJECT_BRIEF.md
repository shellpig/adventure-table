...
最後更新：2026-09-15（P4-C — Attack, Damage & Core Action Resolution 關門；P4-B 關門；P4-A 關門；U01-A 關門）
...
- **尚未實作**：Spell / Condition / Concentration / Reaction resolution、Quick Combat UI、Combat MCP tool 的 UI 接線與 guide（P4-D～P4-F）、Adventure Runtime。Session 頁的 Log 分頁目前仍只是 raw event 列表。
...
1. **產品主線：P4-D — Spells, Conditions, Concentration & Reactions 實作**：在 P4-C 的 semantic `apply_damage` / `apply_healing` boundary 與 formal save substrate 上做 spell skeleton / upcast、Character 與 Monster stat-block casting source、2014 Conditions、Temporary Effects、Concentration canonical state 與受傷後 CON save request（原測試指南 C.4.2，已移至 D.3）、ReactionRequest / Ready、Quick OA 由 DM trigger；P4-C 留下的 Monster 0 HP outcome action、`grappled` escape、Character affinity 欄位在此或 P4-E 決定歸屬；同分支續接 `postgres-migrations` gate。
...
3. P4-D～P4-F 依三份 P4 文件順序逐段實作、review、驗收；P5～P8 仍維持大 Phase，不提前拆分或設計其 schema / API / module。
...
| P4 | Quick Combat | 第一個完整可玩的 Combat MVP；**A～F 已完成拆分與三份正式文件；P4-A、P4-B、P4-C 已於 2026-09-15 關門並合併回 `main`，每個 Subphase 各自合併；下一步 P4-D** |
