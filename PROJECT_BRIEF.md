...
- **P6-A、P6-B 已關門並合併回 `main`**（P6-A 2026-09-20：Room asset substrate、Adventure Definition／Entry authoring 與 lifecycle、Campaign 多重 attach／detach、Adventures UI，證據見 [P6-A closeout](docs/P6/P6-A_CLOSEOUT.md)；P6-B 2026-09-21：migration `0032` 五張 Runtime table、typed Runtime entry／Override／Current Context 與 revision／idempotency、Session 外管理與 active Session Human／AI DM 共用 service、Player secrecy projection、detach blocker、Campaign Changes／Session DM world panel／Player Journal UI，證據見 [P6-B closeout](docs/P6/P6-B_CLOSEOUT.md)）；**P6-C 2026-09-21 關門並合併**（`CampaignContextService` 五個 read intent、bounded Python 端 search、五個 MCP read tool、`get_session_context` compact `campaign_context`，證據見 [P6-C closeout](docs/P6/P6-C_CLOSEOUT.md)）。**下一步為 P6-D — AI DM Write-back & Exploration Integration**，開工前先拆 `campaign_runtime/service.py`（2,511 行）。2026-09-19 使用者拍板讓 P6 先於 P5 執行，以先完成「世界／冒險資料 → Exploration → Quick Combat → 世界狀態跨 Session 保存」的可玩 Campaign loop；Phase 編號不重編，P5 契約完整保留，P6 關門後再回 P5。
- **P6 契約已定案，固定 A～G**；開工讀 [P6 實作規格](docs/P6/實作規格.md)、[開發設計方針](docs/P6/開發設計方針.md)、[測試指南](docs/P6/測試指南.md) 的 P6-D 與必要共用前言。P6-A～P6-C 的技術決策（raw-body asset upload、縮圖走 authenticated blob fetch、duplicate attach 409、context clear 不刪 row、required `TableEventService`、Player Journal 只列 public quest／fact＋own-character knowledge、search 為 Python 端比對且 Player 不觸 Adventure 分支、`_active_context_extension` hook）見各自 closeout。列出下一步不代表 coding 授權。
...
✅＝已關門並合併；⬜＝正式契約已存在、尚未實作。當前 Phase 一列一個 Subphase；完成的 step 狀態只住該 Subphase 實作紀錄，不寫進本檔。
...
| P6 | Adventure Definition／Importer、Campaign Runtime、AI DM context／write-back；**當前 Phase，P6-A～P6-C 已關門，下一步 P6-D** |
...
- **P6 接手**：先交付不依賴 Importer 的可玩 loop（manual Adventure／空 Campaign、Runtime、AI context/write-back）；Importer 後置於 P6-E/F。P6 不放寬 P3-D pre-session AI grant，也不提前做 P5 geometry 或 P7 Timeline／Snapshot。
