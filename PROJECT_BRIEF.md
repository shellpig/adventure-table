...
最後更新：2026-09-15（P4-B — Combat Lifecycle, Initiative & Action Economy 關門；P4-A 關門；U01-A 關門）
...
1. **產品主線：P4-C — Attack, Damage & Core Action Resolution 實作**：在 P4-B 的 `consume_action()` economy validator 與 P4-A 的 canonical `MonsterAction` 上做 Attack / Save / Damage / Healing / Crit / resistance / Temp HP / Grapple / Shove / 0 HP / Death Save 的 atomic idempotent resolve；`attack_budget` 只是次數，to-hit / damage 尚未存在；Monster Multiattack 的 `attacks_allowed` 與 `removed` entry 對 Monster Instance `combat_status` 的連動在此決定；同分支續接 `postgres-migrations` gate。
...
| P4 | Quick Combat | 第一個完整可玩的 Combat MVP；**A～F 已完成拆分與三份正式文件；P4-A、P4-B 已於 2026-09-15 關門並合併回 `main`，每個 Subphase 各自合併；下一步 P4-C** |
...
- **P4 已完成開工前置，P4-A、P4-B 已關門**：P4-A～F 與三份正式文件已於 2026-09-13 定案；Monster source/count 後續校正為同一 pinned corpus **334 筆／Beast 87 筆**，P4-A 已依此交付。P4-A 的 Monster Template content 進共用/standalone ContentRegistry，但 Monster Instance / Combat tables / services 不進 Standalone；Monster long-form `desc` 尚無 zh-TW，第一個 expose 它的 Subphase 必須同步補齊；P4-D spell resolution 同時接受 Character 與 Monster stat-block casting source；Combat canonical state以 Campaign為 lifecycle root、可跨 Session延續（P4-B 已交付：Session End / Abandon 不碰 Combat，entry identity 是 Character / Monster Instance 而非 Seat snapshot）；P4-B 的 `is_hostile` 只由 subject kind 決定，DM 尚不能切換；Quick Combat不做 geometry；敵人精確 HP／AC／hidden resources在 Server projection層對 Player隔離。
