...
最後更新：2026-09-18（P4-F F1～F7c、F9a 完成，剩 F8 真實 ChatGPT gate 與 F9b 關門；P4-E、P4-D、P4-C、P4-B、P4-A、U01-A 已關門）
...
- **網頁版 chat 接入與 AI Join Kit（M04）**：ChatGPT Web Plus 可用 connector + OAuth（authorize 頁貼 AI Join Token）以既有 Seat 的 role 進桌，OAuth token 只對應既有 P3-D grant。Lobby／Session 在發 token 的同一畫面產出可複製／下載的雙語 AI Join Kit（本機／公網雙 URL、role、到期、三種接入方式）；`GET /mcp/guide` 無需認證提供與 tool catalog 同源的完整指引；`get_session_context.briefing` 是強制逐步迴圈；`wait_for_event` 上限 120 秒。
- **Monster & Combatant foundation（P4-A）**：pinned SRD 5.1 Monster corpus 334 筆（Beast 87）已進正式 content pipeline，Monster name 與 Trait / Action / Bonus Action / Reaction / Legendary Action 名稱 `zh-TW` / `en` 齊全；`monster_templates` / `monster_instances` 持久化、Quick Enemy、Save as Template、canonical `MonsterAction`（kind / attack_kind / damage_parts / automation_level）與 DM / Player `CombatantProjector` 已就位；Standalone 不載 Instance / Combat schema。
- **Quick Combat lifecycle（P4-B）**：Campaign-scoped durable Combat（`combats` / `combat_entries` / `combat_actions`），同 Campaign 只允許一場 active（DB partial unique index + 真 PostgreSQL 併發證據），mode 固定 `quick`；initiative 沿用 P3-C formal roll（PC `1d20+DEX`、monster group、advantage / disadvantage、tie 只能由 DM 在同 total 內裁定、mid-combat entrant）；Server authoritative round / turn advance、Action / Bonus / Reaction / Extra Attack / surprise economy、Dash / Disengage / Dodge / Search / Ready / Freeform 最小 representation；Combat 跨 Session End / Abandon 延續，同一 Character 由本場 controller 續控既有 entry，缺席 Character 只有 DM 可 skip / withdraw / remove / proxy；`no_hostile_combatants` 只提示不自動 end；End Combat 只清 combat bookkeeping。已有 16 條 Human HTTP route。
- **Attack / Damage / Core Action Resolution（P4-C）**：Character weapon / feature attack 與 Monster action normalize 成同一 `ResolvedAttack`；attack / save / death save 走 P3-C formal roll（normal / advantage / disadvantage、Nat 20 double dice only、Nat 1 auto miss）；命中後 Server 直接算傷害並在同一 transaction 套用 target state；active Combat 的受傷／治療統一走 semantic `apply_damage` / `apply_healing` boundary（Temp HP 先吸收、Monster resistance / immunity / vulnerability、PC 0 HP → unconscious + prone + death saves、massive damage、Monster 0 HP 只標 `monster_outcome_required`），Player / AI Player 的 raw `current_hp` patch 在 active Combat 被拒，DM 必須帶 `correction_reason` 才能 absolute set；Quick Combat 一律先 `dm_adjudication_required` 再 formal roll；Grapple / Shove opposed check 與 free hand / size / reach constraint；同 idempotency key 不重擲不重扣，真 PostgreSQL 併發與注入失敗 rollback 證據。新增 13 條 Human HTTP route 與 14 個 `combat_*` MCP tool，MCP facade 只 delegate 同一 service。尚無 Combat UI。
- **Spells / Conditions / Concentration / Reactions（P4-D）**：Character spell cast 走既有 spell access / prepared / `spell_slots` / Pact Magic counter，Monster casting source 讀 P4-A stat-block `traits[].spellcasting`；單體 cast（attack / save / heal / utility）與 AoE（acting user 提 target identity → DM confirm → group saves）都是一個 durable transaction；2014 Conditions 全 14 個 + Exhaustion 有 typed mechanical semantics、registry 與 zh-TW；Temporary Effects 為 typed modifier + deterministic expiry；Concentration 是 `CharacterState` canonical state，受傷建正式 CON save、失敗清 pointer 與所有 combatant 上的 linked effects、End Combat 不清；ReactionRequest 是 `combat_entries.pending_reaction_state` 的 durable window，Ready 用同一 substrate，Quick OA 只由 DM trigger。Character Current State additive 擴充（concentration / exhaustion / death saves / temporary effects / condition effect_id），JSON v1 與 Web↔Standalone roundtrip 不變。只到 repository / domain，尚無 REST / MCP / UI。
- **尚未實作**：P4-F 的 full P4 integration（browser spell / reaction journey、Monster reveal / bookkeeping UI、`ConditionSemantics` 接進 attack / save pipeline、真實 ChatGPT Web Combat gate）、Adventure Runtime。
...
- **Quick Combat = Exploration UI + canonical Combat State，不做假 geometry**：P4 不追精準距離、movement feet、Cover、AoE geometry 或 OA geometry；這些由 DM 裁定，P5 才加入 Tactical spatial state。
- **核心戰鬥規則自動化、複雜能力 DM adjudication**：Attack / Save / Damage / Healing / Crit / Advantage / Disadvantage / action economy / Conditions / Concentration / Death Save / resources 由 Server 處理；特殊 Monster ability、複雜 Feat、敘事型 Spell 可 partial structured + description，不做 generic rules DSL。
...
1. **產品主線：P4-F 收尾——F8 真實 ChatGPT Web Combat gate（測試指南 F.3，人工）→ F9b 關門、`--no-ff` 合併回 `main`、P4 Phase closeout**。P4-F 步驟表在 [P4-F實作紀錄.md](docs/P4/P4-F實作紀錄.md)；開工時歸入 P4-F 的 P4-E「已知限制」已全部處理：Monster reveal / position note 修改與 DM bookkeeping UI（實作規格 P4-E 7 只到部分）、browser 層的 spell cast / AoE / reaction journey、`ConditionSemantics` 接進 attack / save modifier pipeline、Monster 0 HP outcome action、`grappled` escape、Character state PATCH DTO 的 P4-D 欄位、真實 ChatGPT Web Combat journey（測試指南 F.3）。
...
3. P4-F 依三份 P4 文件逐段實作、review、驗收；P5～P8 仍維持大 Phase，不提前拆分或設計其 schema / API / module。
...
- [P4 實作規格](docs/P4/實作規格.md)
- [P4 開發設計方針](docs/P4/開發設計方針.md)
- [P4 測試指南](docs/P4/測試指南.md)
- [P4-A closeout](docs/P4/P4-A_CLOSEOUT.md)
- [P4-B closeout](docs/P4/P4-B_CLOSEOUT.md)
- [P4-C closeout](docs/P4/P4-C_CLOSEOUT.md)
- [P4-D closeout](docs/P4/P4-D_CLOSEOUT.md)
- [P4-E closeout](docs/P4/P4-E_CLOSEOUT.md)
...
| P4 | Quick Combat | 第一個完整可玩的 Combat MVP；**A～F 已完成拆分與三份正式文件；P4-A、P4-B、P4-C 已於 2026-09-15、P4-D 於 2026-09-16 關門並合併回 `main`，P4-E 於 2026-09-17 關門並合併回 `main`，每個 Subphase 各自合併；下一步 P4-F** |
...
| **P4-F — Full P4 Integration & Closeout** | 🟡 | F1～F7c、F9a 已完成（Monster outcome / bookkeeping / reveal、escape_grapple、state PATCH P4-D 欄位、真 PostgreSQL restart、full browser journey + Session boundary + 真 process restart、conditions → attack / save modifier 與 auto-fail、migration `0027`～`0029`、static review + 全套 regression）；[closeout 草稿](docs/P4/P4-F_CLOSEOUT.md) 第 9、10 項留白。**剩 F8 真實 ChatGPT Web Combat gate（人工）與 F9b 關門 / 合併**；步驟紀錄 [P4-F實作紀錄.md](docs/P4/P4-F實作紀錄.md) |
...
- **P4 已完成開工前置，P4-A、P4-B、P4-C、P4-D、P4-E 已關門**：P4-A～F 與三份正式文件已於 2026-09-13 定案；Monster source/count 後續校正為同一 pinned corpus **334 筆／Beast 87 筆**，P4-A 已依此交付。P4-A 的 Monster Template content 進共用/standalone ContentRegistry，但 Monster Instance / Combat tables / services 不進 Standalone；Monster long-form `desc` 尚無 zh-TW，第一個 expose 它的 Subphase 必須同步補齊；P4-D spell resolution 同時接受 Character 與 Monster stat-block casting source；Combat canonical state以 Campaign為 lifecycle root、可跨 Session延續（P4-B 已交付：Session End / Abandon 不碰 Combat，entry identity 是 Character / Monster Instance 而非 Seat snapshot）；P4-B 的 `is_hostile` 只由 subject kind 決定，DM 尚不能切換；P4-C 起 active Combat 的 HP 變動只能走 semantic damage / healing boundary，Player / AI Player 的 raw `current_hp` patch 被拒、DM absolute set 必須帶 `correction_reason`，後續 Subphase 不得繞過；Character 抗性尚無 machine-readable 欄位；P4-D 起 Concentration / Exhaustion / Death Save / Temporary Effects 住 shared Character Current State（additive、Standalone 可攜），spell cast 只能經 `authorize_character_spell` → `spend_character_spell`，Concentration CON save 只能由 `CombatConcentrationRepository.complete_check` 消費，Monster caster persisted cast 與 Monster concentration 留 P4-E；Quick Combat不做 geometry；敵人精確 HP／AC／hidden resources在 Server projection層對 Player隔離；P4-E 起所有 Combat service mutation 必須喚醒 long-poll waiters（`TableEventService.notifier`），前端把帶 `combat_id` 的 `roll.*` event 視為 Combat state change。
...
歷史完成過程與驗收證據查各 Phase 目錄的 `*_CLOSEOUT.md`；M01-B 真人創角 Gate 另見 [M01-B_HUMAN_GATE.md](docs/M01/M01-B_HUMAN_GATE.md)。最近**已完成產品Phase**的整合交付見 [P2-F_CLOSEOUT.md](docs/P2/P2-F_CLOSEOUT.md)；P3 Subphase closeout 為 [P3-A](docs/P3/P3-A_CLOSEOUT.md)、[P3-B](docs/P3/P3-B_CLOSEOUT.md)、[P3-C](docs/P3/P3-C_CLOSEOUT.md)、[P3-D](docs/P3/P3-D_CLOSEOUT.md)、[P3-E](docs/P3/P3-E_CLOSEOUT.md) 與 [P3-F](docs/P3/P3-F_CLOSEOUT.md)；P3 整合交付見 P3-F closeout；M04 為 [M04-A_PREFLIGHT](docs/M04/M04-A_PREFLIGHT.md)、[M04-B](docs/M04/M04-B_CLOSEOUT.md) 與 [M04-C](docs/M04/M04-C_CLOSEOUT.md)；P4 Subphase closeout 為 [P4-A](docs/P4/P4-A_CLOSEOUT.md)、[P4-B](docs/P4/P4-B_CLOSEOUT.md)、[P4-C](docs/P4/P4-C_CLOSEOUT.md) 與 [P4-D](docs/P4/P4-D_CLOSEOUT.md)。
