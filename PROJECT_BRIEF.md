...
最後更新：2026-09-19（P4-F 關門並合併回 `main`，P4 Phase 關門；U01-A 已關門；下一步 P5 開工前置）
...
- **Full P4 integration（P4-F）**：Monster 0 HP outcome / reveal / bookkeeping、escape_grapple、真 PostgreSQL restart 恢復、full browser journey + Session boundary + 真 process restart、conditions / exhaustion / Dodge → attack / save modifier 與 auto-fail、Quick Enemy attack 輸入正規化、AI `next_required_action` 依 pending 裁定分流與 `advance_turn`、真實 ChatGPT Web Combat gate 三場；migration `0027`～`0030`。
- **尚未實作**：Tactical geometry（P5）、Adventure Runtime（P6）。
...
1. **產品主線：P5 — Tactical Combat 開工前置**：依 `AGENTS.md` Phase 原則，先以 P4 實際 codebase 為基礎拆 `P5-A`… Subphase 並寫三份 P5 文件，再 coding；P4-F closeout「已知限制」中歸 P5 一併看的項目（Quick Combat 每次近戰 range 裁定、Dodge「能看見攻擊者」、frightened 來源可見性）在拆分時納入評估。P4-F 步驟表在 [P4-F實作紀錄.md](docs/P4/P4-F實作紀錄.md)；開工時歸入 P4-F 的 P4-E「已知限制」已全部處理：Monster reveal / position note 修改與 DM bookkeeping UI（實作規格 P4-E 7 只到部分）、browser 層的 spell cast / AoE / reaction journey、`ConditionSemantics` 接進 attack / save modifier pipeline、Monster 0 HP outcome action、`grappled` escape、Character state PATCH DTO 的 P4-D 欄位、真實 ChatGPT Web Combat journey（測試指南 F.3）。
...
3. P6～P8 仍維持大 Phase，不提前拆分或設計其 schema / API / module；P4 留下的非 P5 項目（spell save 不走 conditions pipeline、`get_resolution_event` O(n)、Monster `desc` 未 expose、死亡 vs 倒地不起標籤）歸後續 M Phase。
...
| P4 | Quick Combat | 第一個完整可玩的 Combat MVP；**A～F 全數關門並各自合併回 `main`（P4-A～C 2026-09-15、P4-D 09-16、P4-E 09-17、P4-F 09-19），Phase 已關門**；真實 ChatGPT Web Combat gate 三場通過 |
