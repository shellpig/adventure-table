...
最後更新：2026-09-16（P4-D — Spells, Conditions, Concentration & Reactions 關門；P4-C、P4-B、P4-A、U01-A 已關門）
...
- **尚未實作**：Quick Combat UI、Combat spell / reaction / concentration 的 REST / MCP 接線與 guide、Monster caster persisted cast、Monster concentration（P4-E～P4-F）、Adventure Runtime。Session 頁的 Log 分頁目前仍只是 raw event 列表。
...
1. **產品主線：P4-E — Quick Combat UI, DM Adjudication & AI Tool Surface 實作**：把 P4-A～D 的 Combat Engine 接成真人與外部 AI 都能跑的桌面體驗：Session table Quick Combat UI、enemy secrecy DTO、range / cover / AoE / OA adjudication UX、Combat spell / reaction / concentration 的 REST + MCP tool、guide / briefing 雙語。P4-D 交接的三項已寫進正式契約：Monster caster persisted cast（實作規格 P4-E 14）、Monster concentration canonical state（15）、Concentration CON save 只能由 `CombatConcentrationRepository.complete_check` 消費（16）；P4-C 留下的 Monster 0 HP outcome action、`grappled` escape、Character affinity 欄位在此決定歸屬；同分支續接 `postgres-migrations` gate。
...
3. P4-E～P4-F 依三份 P4 文件順序逐段實作、review、驗收；P5～P8 仍維持大 Phase，不提前拆分或設計其 schema / API / module。
...
| P4 | Quick Combat | 第一個完整可玩的 Combat MVP；**A～F 已完成拆分與三份正式文件；P4-A、P4-B、P4-C 已於 2026-09-15、P4-D 於 2026-09-16 關門並合併回 `main`，每個 Subphase 各自合併；下一步 P4-E** |
