...
1. **產品主線：P5 — Tactical Combat 開工**：三份 P5 文件已於 2026-09-19 建立（`a9586d99`，Subphase 固定為 P5-A～P5-G），下一步依 `docs/P5/實作規格.md` §5 從 `P5-A — Battle Map & Spatial Foundation` 開工；P4-F closeout「已知限制」中歸 P5 一併看的項目（Quick Combat 每次近戰 range 裁定、Dodge「能看見攻擊者」、frightened 來源可見性）在拆分時納入評估。P4-F 步驟表在 [P4-F實作紀錄.md](docs/P4/P4-F實作紀錄.md)；開工時歸入 P4-F 的 P4-E「已知限制」已全部處理：Monster reveal / position note 修改與 DM bookkeeping UI（實作規格 P4-E 7 只到部分）、browser 層的 spell cast / AoE / reaction journey、`ConditionSemantics` 接進 attack / save modifier pipeline、Monster 0 HP outcome action、`grappled` escape、Character state PATCH DTO 的 P4-D 欄位、真實 ChatGPT Web Combat journey（測試指南 F.3）。
