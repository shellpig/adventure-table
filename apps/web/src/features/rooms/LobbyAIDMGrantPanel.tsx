...
export function LobbyAIDMGrantPanel({ roomId, campaignId, seat, roomToken, copy, onChanged }: LobbyAIDMGrantPanelProps) {
  ...
  const issue = async () => {
    ...
    try {
      const next = await configurePreSessionAiDm(roomId, campaignId, seat.id, roomToken)
      setIssued(next); await onChanged()
    } catch { setError(copy.aiDmFailed) } finally { setPending(false) }
  }
  ...
  const revoke = async () => {
    ...
    try {
      ...
      setIssued(null); setExpired(false); await onChanged()
    } catch { setError(copy.aiDmFailed) } finally { setPending(false) }
  }
  ...
  const copyToken = async () => {
    if (!issued) return
    try {
      await navigator.clipboard.writeText(issued.token); setCopied(true); setTimeout(() => setCopied(false), 2500)
    } catch { setError(copy.aiDmCopyFailed) }
  }

  const configured = seat.controller_kind === 'ai'
  return (
    <section className="room-form" data-ai-dm-grant-panel={seat.id}>
      <h3>{copy.aiDmTitle}</h3><p>{copy.aiDmFiniteHint}</p>
      {configured ? <p><strong>{copy.aiDmConfigured}</strong></p> : null}
      <div className="workshop-card__split-actions">
        <button className="button secondary" type="button" disabled={pending} onClick={() => void issue()}>{configured ? copy.aiDmRotate : copy.aiDmCreate}</button>
        {configured ? <button className="button danger" type="button" disabled={pending} onClick={() => void revoke()}>{copy.aiDmRevoke}</button> : null}
      </div>
      {issued ? (
        <div className="notice-banner" role="status" data-ai-dm-token-once="true">
          <strong>{copy.aiDmTokenOnceTitle}</strong><p>{copy.aiDmTokenOnceHint}</p>
          {issued.expires_at ? <p>{copy.aiDmExpiresAt}: <time dateTime={issued.expires_at}>{issued.expires_at}</time>{expired ? ` · ${copy.aiDmExpired}` : ''}</p> : null}
          <div className="token-display-box">
            <textarea readOnly rows={2} value={issued.token} aria-label={copy.aiDmTokenOnceTitle} className="token-display-input" onClick={(event) => (event.target as HTMLTextAreaElement).select()} />
