...
function getStatusLabel(status: string, copy: SessionCopy): string {
  switch (status) {
    case 'active':
      return copy.combatStatusActive
    case 'withdrawn':
      return copy.combatStatusWithdrawn
    case 'down':
      return copy.combatStatusDown
    case 'dead':
...
export function SessionCombatStage({
  ...
  return (
    <section className="session-combat" aria-label={copy.combatTitle}>
      <header
        className="session-combat__header"
        data-combat-round={combat.round_number ?? 'pre-initiative'}
        data-combat-current-turn={currentTurnName}
      >
        <div className="session-combat__round-indicator">
          <span className="session-combat__round-badge">{roundText}</span>
        </div>
        <div className="session-combat__turn-indicator">
          ...
          {isMyTurn ? (
            <span className="session-combat__your-turn-badge">{copy.combatYourTurn}</span>
          ) : null}
        </div>
      </header>
        ...
        <ol className="session-combat__initiative-list">
          {entries.map((entry) => {
            ...
            return (
              <li
                key={entry.id}
                className={`session-combat__initiative-row${isCurrent ? ' session-combat__initiative-row--current' : ''}${isHostile ? ' session-combat__initiative-row--hostile' : ''}`}
        ...
        {combatantEntries.length === 0 ? (
          ...
          <div className="session-combat__cards-grid">
            {combatantEntries.map(({ entry, combatant }) => {
              ...
              const conditions = proj.conditions
                .map(conditionLabel)
                .filter((label) => label.length > 0)
              let hpDisplay: string | null = null
              if (typeof proj.current_hp === 'number') {
                hpDisplay =
                  typeof proj.max_hp === 'number'
                    ? `${proj.current_hp}/${proj.max_hp}`
                    : `${proj.current_hp}`
                if (typeof proj.temp_hp === 'number' && proj.temp_hp > 0)
                  hpDisplay += ` (+${proj.temp_hp})`
              }
              const hasAc = typeof proj.armor_class === 'number'
              const hasPositionNote =
                typeof proj.position_note === 'string' && proj.position_note.trim().length > 0
              const hasDmNotes =
                typeof proj.dm_notes === 'string' && proj.dm_notes.trim().length > 0
              return (
                <div
                  key={entry.id}
                  className={`session-combat__card${isHostile ? ' session-combat__card--hostile' : ''}`}
                  data-combat-entry={entry.id}
                  data-hostile={isHostile ? 'true' : undefined}
                >
                  ...
                  <div className="session-combat__economy">
                    <span
                      className={`session-combat__economy-pill${entry.action_available ? ' session-combat__economy-pill--available' : ''}`}
                    >
                      {copy.combatActionShort}
                    </span>
                    <span
                      className={`session-combat__economy-pill${entry.bonus_action_available ? ' session-combat__economy-pill--available' : ''}`}
                    >
                      {copy.combatBonusActionShort}
                    </span>
                    <span
                      className={`session-combat__economy-pill${entry.reaction_available ? ' session-combat__economy-pill--available' : ''}`}
                    >
                      {copy.combatReactionShort}
                    </span>
                    {entry.attacks_allowed > 1 ? (
                      <span className="session-combat__attacks">
                        {entry.attacks_used}/{entry.attacks_allowed} {copy.combatAttacks}
                      </span>
                    ) : null}
                  </div>
                  <div className="session-combat__stats-row">
                    {hpDisplay !== null ? (
                      <span className="session-combat__stat">
                        <span className="session-combat__stat-label">{copy.combatHp}:</span>
                        <span>{hpDisplay}</span>
                      </span>
                    ) : null}
                    {hasAc ? (
                      <span className="session-combat__stat">
                        <span className="session-combat__stat-label">{copy.combatAc}:</span>
                        <span>{proj.armor_class}</span>
                      </span>
                    ) : null}
                    {proj.injury_level ? (
                      <span className="session-combat__stat">
                  ...
                  {conditions.length > 0 ? (
                    <div className="session-combat__conditions">
                      {conditions.map((conditionName, index) => (
                        <span
                          key={`${conditionName}-${index}`}
                          className="session-combat__condition-pill"
                        >
                          {conditionName}
                        </span>
                      ))}
                  ...
                  {hasPositionNote ? (
                    <div className="session-combat__position-note">
                      <span className="session-combat__stat-label">{copy.combatPositionNote}:</span>
                      <span>{proj.position_note}</span>
                    </div>
                  ) : null}
                  {hasDmNotes ? (
                    <div className="session-combat__dm-notes">
