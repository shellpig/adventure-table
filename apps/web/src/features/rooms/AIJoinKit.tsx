...
const templateCopy = {
  'zh-TW': {
    ...
    token: 'Token',
    expires: 'Expires',
    expiresFallback: '直到跑團結束或被撤銷',
    aiDirective: 'AI 請直接照做：你已透過 MCP 連上這張桌，不需要、也不能自己設定 connector。你的第一個動作就是「實際呼叫」get_session_context 工具（真的 tool call，不是嘴上說要呼叫），再依回傳的 briefing 進行。除非某次實際 tool call 回傳錯誤，否則不得宣稱連線失敗；還沒呼叫過就說「尚未測試」。工具呼叫偶爾會連續失敗 2–3 次（逾時、connector 錯誤），這是暫時的，稍後就會恢復：直接重試同一個呼叫直到成功，不要停下來、也不要把控制交回人類。',
    ...
    refresh: '跑團開始前後工具清單相同，start_session 後不需 Refresh。換 Role／席位必須重新授權；之後工具仍是舊清單就 Refresh／重新掃描 connector 並開新對話。',
    web: '網頁版 AI agent：由使用者在 AI agent 的 connector 設定新增 Adventure Table connector，URL 填「公網」URL，OAuth 頁貼上此 token。',
  ...
  en: {
    ...
    aiDirective: 'AI, do this now: you are already connected over MCP — you do not and cannot set up a connector yourself. Your first action is to actually CALL the get_session_context tool (a real tool call, not a statement that you will), then follow the briefing it returns. Never claim the connection failed unless a real tool call returned an error; if you have not called a tool yet, say it is untested. Tool calls sometimes fail 2–3 times in a row (timeout, connector error); this is temporary and recovers shortly, so retry the same call until it succeeds — do not stop and do not hand control back to the human.',
    ...
    web: 'Web chat AI agent: the user adds the Adventure Table connector in the AI agent\'s connector settings with the remote URL and pastes this token on the OAuth page.',
