...
describe('M04-C AI Join Kit', () => {
  it('builds a complete English DM kit with local and remote URLs', () => {
    ...
    expect(kit).toContain('Web chat AI agent: the user adds the Adventure Table connector')
    expect(kit).toContain('retry the same call until it succeeds')
    expect(kit).not.toContain('ChatGPT')
  ...
  it('builds a Traditional Chinese Player kit and explains the missing remote URL', () => {
    ...
    expect(kit).toContain('網頁版 AI agent：由使用者在 AI agent 的 connector 設定新增 Adventure Table connector')
    expect(kit).toContain('直接重試同一個呼叫直到成功')
    expect(kit).not.toContain('ChatGPT')
