...
class MCPToolDefinition:
    ...
    def rich_description(self) -> str:
        ...
        parameters = ", ".join(props) if props else "none"
        required_text = ", ".join(required) if required else "none"
        role_text = ", ".join(sorted(self.roles))
        return (
            f"{self.description} When to use: call this only when the current table state requires {self.name}. "
            f"Key parameters: {parameters}; required: {required_text}. Allowed roles: {role_text}. "
            "Respect the current scoped Seat, returned cursor/state, schema enum values, and idempotency fields when present. / "
            f"使用時機：目前桌面流程需要 {self.name} 時才呼叫。關鍵參數：{parameters}；必填：{required_text}；可用角色：{role_text}。"
            "必須遵守目前 scoped Seat、回傳的 cursor／state、schema 合法列舉值，以及存在時的 idempotency 欄位。"
        )
...
def tool_catalog(auth: AIControllerAuthView) -> list[dict[str, Any]]:
    ...
    return [
        definition.wire()
        for definition in _TOOL_DEFINITIONS
        if (not definition.pre_session or definition.name == "get_session_context") and auth.role in definition.roles
    ]
