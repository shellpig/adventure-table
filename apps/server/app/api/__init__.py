...
# counting import/export as separate distribution-level routers. The legacy
# exported router aliases remain public so existing imports are not broken.
characters_router = APIRouter()
characters_router.include_router(_character_core_router)
characters_router.include_router(character_export_router)
characters_router.include_router(character_import_router)
