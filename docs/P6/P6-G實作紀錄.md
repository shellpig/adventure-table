...
- **最近已驗證 commit**：`a1994360`（G2b-3 code；指揮者完成 Docker E2E、frontend、code quality 與 diff 審核）。
- **下一步**：G3a 真 PostgreSQL restart／reconnect，驗 Runtime、Current Context、Import Draft 的資料／revision／visibility；由 agy 實作，指揮者審核驗證。真實 AI DM context 需合法 controller grant，留 G4。
