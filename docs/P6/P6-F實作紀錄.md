...
- **下一步**：F2 Finalize transaction；尚未派工。
- **阻礙／未審**：無。
...
- **跨步依賴**：F1 → F2 → F3 → F4／F5（F4 與 F5 都只依賴 F2／F3，可並行但本排程照序）。P6-E 交付面（`AdventureImportService`、`ImportDraft` schema、REST router、web API client）見 [P6-E closeout](P6-E_CLOSEOUT.md)；MCP facade 模式與 `require_active_table_actor` 見 [P6-D closeout](P6-D_CLOSEOUT.md)。
...
| F2 | Finalize transaction：blocking gate、draft → Adventure Definition／entries、`target_adventure_id` idempotency | 待做 | F1 | [F2](P6-F_steps/F2.md) |
| F3 | REST：review／resolve／answer／finalize route | 待做 | F2 | [F3](P6-F_steps/F3.md) |
| F4 | 六個 Importer MCP tool＋catalog／guide parity＋authority | 待做 | F2 | [F4](P6-F_steps/F4.md) |
