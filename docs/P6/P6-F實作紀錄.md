...
- **下一步**：F4 已拆為 F4a（AI DM authority 接線）／F4b（六個 MCP tool）；先派 agy 做 F4a。finalize 的派工決策（`dm_only` 預設、draft revision、attachment role）見 [F2b](P6-F_steps/F2b.md)；F5 須補 `adventure_import_blocking_warnings` 的 zh-TW／en 文案（見 [F3](P6-F_steps/F3.md)）。
- **阻礙／未審**：無。
...
| F4a | Importer 的 current AI DM authority 接線 | 進行中 | F3 | [F4a](P6-F_steps/F4a.md) |
