...
- **最近已驗證 commit**：`87246094`（F1 code；指揮者完成 focused／P6-E Importer regression 與 diff 審核）。
- **下一步**：F2 已拆為 F2a／F2b；先派 agy 做 F2a transaction 接線。
- **阻礙／未審**：無。
...
- **跨步依賴**：F1 → F2a → F2b → F3 → F4／F5（F4 與 F5 都只依賴 F2b／F3，可並行但本排程照序）。P6-E 交付面（`AdventureImportService`、`ImportDraft` schema、REST router、web API client）見 [P6-E closeout](P6-E_CLOSEOUT.md)；MCP facade 模式與 `require_active_table_actor` 見 [P6-D closeout](P6-D_CLOSEOUT.md)。
...
| F2a | Adventure authoring 共用 transaction 接線 | 進行中 | F1 | [F2a](P6-F_steps/F2a.md) |
| F2b | Import Finalize transaction／blocking gate／idempotency | 待做 | F2a | [F2b](P6-F_steps/F2b.md) |
| F3 | REST：review／resolve／answer／finalize route | 待做 | F2b | [F3](P6-F_steps/F3.md) |
| F4 | 六個 Importer MCP tool＋catalog／guide parity＋authority | 待做 | F2b | [F4](P6-F_steps/F4.md) |
