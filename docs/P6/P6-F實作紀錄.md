...
- **最近已驗證 commit**：`cbfa5297`（F3 code；指揮者完成 P6 全部 backend regression、web 全部單元測試與 build、diff 審核）。
- **下一步**：F4 六個 Importer MCP tool。finalize 的派工決策（`dm_only` 預設、draft revision、attachment role）見 [F2b](P6-F_steps/F2b.md)；F5 須補 `adventure_import_blocking_warnings` 的 zh-TW／en 文案（見 [F3](P6-F_steps/F3.md)）。
- **阻礙／未審**：無。
