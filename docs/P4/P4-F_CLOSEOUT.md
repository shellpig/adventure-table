...
> **狀態：closeout gates 全部完成（F9b，2026-09-19）。** F8 真實 ChatGPT Web Combat gate 三場（第一場抓到 F8a / F8b 缺陷，修正後第二／三場通過），F8c 收掉回饋，F9b 以 code SHA `651a14d0` 重跑全套 regression 與本機全套 E2E 後關門、`--no-ff` 合併回 `main`。
...
- [x] 5. Player / AI Player 敵人 secrecy 三層：browser（F6b / E2E 第 7 點 Player 卡片與 detail API 無 HP / AC；F3b Player 無 `[data-monster-controls]`）、API（`test_p4e_combat_detail_secrecy.py`、`test_p4e_event_secrecy.py` forbidden_keys 含 F3a `reveal`）、MCP（`test_p4e_mcp_combat_context_and_effects.py`、`test_p4e_session_context_combat.py`）。真實 gate：F8 第三場 Player 截圖 Goblin 卡片無 HP / AC；本場 hit payload 經 `project_combat_event_payload(audience="player")` 投影後無 `before` / `after` / `target_ac` / `formula`（實作紀錄「F8 — 第二／三場」）。
- [x] 6. DM proxy Player action 同一 rules / economy 且 audit 保留 acting DM 與 subject：`_authorize_roll` 回 `dm_proxy` execution_mode（`test_p4c_core_rolls.py`、`test_p4f_auto_fail_saves.py` DM 替 Monster 擲）；`test_p3c_*` execution_mode audit 未變。真實 gate：F8 第一場 seq 7 `roll.resolved execution_mode=dm_proxy`，acting seat `cb66c8f1`（AI DM）/ subject `4f0a6503`（Player）——AI DM 代擲玩家先攻，audit 兩者俱存。
- [x] 7. P4 migration 真 PostgreSQL fresh + legacy；standalone boundary 乾淨：`test_p4f_postgres_migration.py` 5（0026 parent → heads、fresh heads、0028 / 0029 / 0030 legacy upgrade / downgrade）；`test_p4a_*` / `test_p4b_*` / `test_p4c_*` / `test_p4e_*` postgres_migration 的 `*_APPLIED_HEADS` 均含 0030；`test_p3c_migration_contract.py` / `test_p3d_migration_contract.py` / `test_m04b_oauth_schema.py` 鎖 0027 → 0028 → 0029 → 0030 鏈；`test_m03_import_boundary.py` 通過（roll_requests / combat tables 皆多人層）。
- [x] 8. P0～P3 / M02～M04 regression 全綠：見 Verification evidence 的全套 backend pytest 與全套 E2E。
- [x] 9. 真實 ChatGPT Web Plus 經 M04 connector 完成 Combat journey：2026-09-19 三場（Room `666` / Campaign `test-01`；client ChatGPT Web Plus，connector 經 Tailscale Funnel `/mcp` + OAuth DCR / PKCE；角色 AI DM，Human Player「玩家1」）。第一場 Session `fae793b5` Combat `eaef0973`：get_session_context → start_session → `combat.started` → Quick Enemy + Character → initiative（formal，含 dm_proxy）→ Player attack ×5（range 裁定 → formal roll → damage，Goblin 7→2→0）→ `turn_advanced` ×8 → outcome dead → `combat.ended`；所有桌上輸出皆為 `session_events` / `combat_actions` 落地，非 host chat。第二／三場 Session `72b3093d`（修正後）補 monster formal attack ×4、`saves_requested` → `save_resolved` → `damage_applied`、Dodge → attack disadvantage / DEX save advantage。實錄見實作紀錄「F8」三段。
- [x] 10. 真實 AI gate 含 `wait_for_event` / reconnect continuity，M04 catalog / auth / role scope 未破壞：第一場 82 次 `POST /mcp` 200、AI 每次 Player 出手後 10～50 秒接手（seq 18→19、26→27、34→35、42→43）；第二場前 connector Refresh + 新對話重掃 catalog（F8a-2 schema 變更後）仍正常；Owner Abandon → grant 與 OAuth authorization 同 timestamp revoke → AI 下一次 `wait_for_event` `permission_denied`（同 M04-B B.8）。`wait_for_event` 呼叫本身為 ChatGPT 自述 + 事件時間差旁證（access log 不記 tool name）。靜態面 `test_m04c_tool_descriptions.py` / `test_m04c_guide_tool_parity.py` / `test_m04c_briefing.py` 通過。**tool error 實錄**：第一場 `combat_resolve_adjudication` ×2 → `table_conflict`（→ F8a-1）、`combat_list_attacks` → `[]`（→ F8a-2）；第二／三場無 tool error。
- [x] 11. static review、Non-E2E regression、Full-Stack E2E 與具名 closeout evidence：本檔（F9a + F9b 兩輪）。

## 契約以外的實作決定

| 項目 | 決定 |
|---|---|
| Migration | web track 四個：`0027_p4f_monster_outcome`（放寬 `combat_entries.status` / `monster_instances.combat_status` 字彙）、`0028_p4f_monster_reveal_state`（`monster_instances.reveal_state` JSON NOT NULL `{}`）、`0029_p4f_roll_request_auto_fail`（`roll_requests.auto_fail` Boolean NOT NULL false）、`0030_p4f_entry_dodging`（`combat_entries.dodging` Boolean NOT NULL false，F8b-1）。Character track 不動（仍 `0015`）。 |
| Dodge（F8b） | canonical entry state：`use_action(dodge)` 設 `dodging`，與 `ready_state` 同一批 UPDATE 清除（dodger 自己下一 Turn 開始、initiative finalize、withdraw / remove / outcome、End Combat）；攻擊它 disadvantage、它的 DEX save advantage，target 有 `blocks_actions` / `speed_zero` condition 即失效；「能看見攻擊者」不建模，交 DM。兩 audience 皆可見（web「閃避中 / Dodging」標籤）。 |
| Quick Enemy attack 輸入（F8a-2） | `attack_kind` Literal（`melee` / `ranged` alias → `*_weapon`），非法值 422 / `invalid_arguments`；`damage` 接受 `1d6+2 slashing`，`split_damage_expression` 拆到 `damage_type`。 |
| AI 引導（F8a-1 / F8c） | `next_required_action` 依 pending kind 分流四個裁定工具、Player 回合 done 回 `advance_turn`；`table_conflict` 帶 `error.detail`；DM combat loop briefing / guide 同步；`BRIEFING_MAX_CHARS` 2,400 → 3,000。 |
...
| Worker | F1～F7c-1、F8a-1 / F8a-2 / F8b-1 agy（Gemini 3.8 Flash (High)），F3a / F3b / F6 / F7c-2 / F8b-2 / F8c / F9 指揮者；F8 人工（使用者 + ChatGPT Web）；agy 監看與判死規則寫進 `docs/others/conductor-handbook.md` §3.1（F7b 第一回合 35 分鐘空跑後新增）。 |
...
Real ChatGPT Web Combat gate（測試指南 F.3）
  見第 9、10 項與實作紀錄「F8」三段；原始 dump C:\_work\AI_Work\Tools\agy-runs\f8-evidence-2026-09-19.txt（repo 外）
```

## Verification evidence（F9b，code SHA `651a14d0`）

F8a～F8c 之後產品碼再動，關門 gate 於 `651a14d0` 重跑：

```text
Alembic heads
  0015_character_state_revision (character) (head)
  0030_p4f_entry_dodging (web) (head)

Backend pytest（全套，cwd apps/server，P4_POSTGRES_URL → adventure_table_p4f）
  651a14d0 本機：1809 passed / 39 skipped，exit 0（12:04）
  39 skip 全為 P2 / P3 / M03 的 PostgreSQL env gate；P4 相關 0 skip（含 test_p4f_postgres_migration 5、restart_recovery 1、concurrency）

Frontend unit / build（651a14d0，本機）
  vitest 86 files / 484 passed；vite build success

docker compose config
  exit 0（本機）

E2E（本機 npm run test:e2e:docker，全套，rebuild server-e2e + web-e2e，adventure_table_e2e 落到 0030）@ 651a14d0
  主套件 129 tests：125 passed / 4 skipped（10.1m，與 full backend pytest 並行執行；p4e-quick-combat 本輪未 flake）
  disabled-pack 套件（xge-less server-e2e，e2e/m03c-character-import.spec.ts）7 tests：7 passed（6.7s）
  exit 0
```

## 已知限制 / 留給後續

- **Quick Combat 每次近戰都要 range 裁定**：無 geometry 時每筆 Player attack 都停在 `dm_adjudication_required`，DM（含 AI DM）每回合要按一次 in-range（F8 第三場 ChatGPT 回饋 4）。規格企劃「Quick：無 geometry，由 DM trigger」的直接後果；「近距離對峙後沿用上次裁定」留 P5 Tactical 一併看。
- **Quick Enemy 沒有屬性值 / 豁免修正**：實作規格 P4-A 7 最低欄位，DEX save 基礎修正為 0；要真 SRD 怪用 `combat_create_monster`（F8 回饋 8）。
- **DM 判 `dead` 後 Player 卡片仍顯示「倒地不起」**：先攻列用 entry status（死亡），卡片用投影 injury level（down）；語意都對但兩處不一致，幾行可修，留後續。
- **AI 不會被主動叫醒**：產品守則 4（網站不接 LLM API），AI DM loop 靠 `wait_for_event` long-poll；AI 回到 host chat 就停桌（F8 回饋 6、9、10、12），F8c 只能在 briefing 強化文字。
- **`wait_for_event` 呼叫無 server 側硬證據**：access log 不記 tool name；第 10 項以 ChatGPT 自述 + 事件時間差為證。
- **M04 文件的 `BRIEFING_MAX_CHARS` 數字**：`docs/M04/測試指南.md` / `開發設計方針.md` 寫死 2,400，F8c 改 3,000，F9b 同步。
