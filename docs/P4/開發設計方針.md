...
- Character spellcasting source：從 Character Build / Current State / spell resource pools解析 spellcasting ability、attack bonus、save DC與可用資源。
- Monster stat-block source：從 resolved Monster Template / Monster Instance stat block解析既有 spellcasting ability、spell attack bonus、save DC與 usage / slot資訊；**不得為 Monster偽造 Character class spellcasting source**。若 stat block已有 explicit DC / attack modifier，normalized Monster spellcasting data就是該來源的 authoritative value（除非未來有正式 instance override）。
...
### 8.5 P4-D 交接：Monster casting、Monster concentration、Concentration roll routing

P4-D 的 persisted spell / concentration / reaction boundary 住在 `app/persistence/combat/spells.py`、`concentration.py`、`reactions.py`、`effects.py`，每個 command 都是一個 `TableEventRepository.append(transaction_projection=...)`。P4-E 接線時：

- **Character cast**：直接呼叫 `CombatSpellRepository.cast_character_spell`（single target / self / utility）與 `propose_character_aoe` / `resolve_character_aoe`（AoE）。`SpellResolutionSpec` 的 cast mode / DC / attack modifier / effects 由 service 從 content spell definition 與 Character rules 解析後傳入；repository 不查 content。
- **Monster cast**：新增 `cast_monster_spell`，走 `resolve_monster_spell_source` + `resolve_monster_spell`，扣 `monster_instances.resources`，target 寫入與 roll record 沿用 Character 版的同一段 projection；不得為 Monster 偽造 Character profile。
- **Monster concentration**：在 `monster_instances` 加 concentration 欄位（JSON，同 `CharacterConcentrationState` 形狀）；`CombatResolutionRepository.apply_damage` 的 monster 分支比照 character 分支建立 `Concentration` roll request；`CombatConcentrationRepository.complete_check` 擴充接受 Monster target。linked effects 的清除沿用 `effects.strip_linked_effects`。
- **Concentration roll routing**：`roll_groups.label == "Concentration"` 且 `ability_ref == "srd5.1:ability:constitution"` 的 pending request 只能由 `CombatConcentrationRepository.complete_check` 解決；P3-C 通用 roll resolve 對這類 request 必須拒絕或轉送，並有測試證明通用路徑不會 resolve 它。
- **Effect 落點**：single-target cast 的 `applied_effects` 落在 target（Character `temporary_effects` + `conditions[].effect_id`；Monster `effects[]` + `conditions[].effect_id`），caster 只保存 `concentration.effect_ids`。UI 顯示 condition 時以 `condition_ref` 為準，不解析 `effect_id`。
...
- 共用 `data/srd5.1/` 的 **Monster Template content可以存在於 standalone ContentRegistry**；content registry載入 Monster definition本身不等於 Standalone支援 Combat。
- Monster Instance / Combat persistence tables、multiplayer routes / services不得進 Standalone schema或 import graph。
- `app.domain.combat`若屬 multiplayer，加入 standalone forbidden import coverage；boundary test / regex應針對 multiplayer module / table，不得因名稱含 `monster` 就 blanket reject共用 Monster Template content。
