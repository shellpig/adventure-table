...
|---|---|---|---|---|---|
| 2026-09-13 | 使用者 Windows 開發機 | 舊版 `test:e2e:docker`（daily `server` / `web`、`adventure_table`） | **10.1m**（127 tests / 1 worker） | 未量 | M01-O closeout，commit `56310b0`。**該證據只有 Playwright 自報的主套件時間**，不含 rebuild 與 xge-less 第二輪；開工時本表誤記為「含 rebuild 與第二輪」，2026-09-14 依原始證據更正口徑 |
| 2026-09-14 | GitHub Actions `ubuntu-24.04` | 新版 isolated `test:e2e:docker`，fresh CI，旁路 daily sentinel | — | 約 **16 分 15 秒** | run `34800987506`；只證明 CI correctness，**不可**與 Windows 數字比較 |
| 2026-09-14 | 同一台使用者 Windows 開發機 | 新版 isolated `test:e2e:docker`，**首次**（createdb + `server-e2e` image 無 layer cache 全量 pip install） | **10.0m**（122 passed / 1 failed / 4 skipped） | **647s**（主套件失敗，未進第二輪） | 失敗為 `m01i` flaky，見 §14.7 與 `已知問題.md` KI-P1D-001 |
