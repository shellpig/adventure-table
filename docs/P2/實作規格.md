...
10. **Standalone boundary 仍是永久 gate。** P2 新增多人模組後，standalone frozen build 與 import graph 必須證明 Character distribution 沒被 Room dependency 汙染。
...
- DM Key：取得 DM 管理／桌內裁定權限。
- Owner Key：管理 Room lifecycle、keys、Campaign 管理與 destructive Room action。
...
- Session active 後繞過 Seat / DM scope 修改別人的 live table state。
- destructive Room / Campaign 管理 action 被一般 member 執行。
...
4. Web 首頁只提供 Create / Enter Room 與 optional Recent Rooms，不再提供 global Character Workshop。
...
11. P2-A 不建立 Campaign、Seat、Session business logic。
...
9. Standalone 仍可使用 Room-less Character Workshop、Draft、Sheet、Version History、Import / Export。
10. Room Hard Delete 可完整刪掉它管理的 Character / Draft，不留下 orphan。
11. P2 升級前既有 global Character / Draft 不得 silent delete；migration / first-room claim 必須有 deterministic upgrade path。
12. 個別 Character 若已被 Campaign / Session history reference，後續不得永久刪除破壞 reference；使用 Archive 保留。Room Hard Delete 是 Owner 對整個 workspace 的 destructive exception。
...
10. Room member / DM / Owner permission boundary 由 Server enforce，不能只靠 frontend 隱藏按鈕。
...
1. Fresh DB migration 到 P2 head 成功。
2. 從現行 `0008_m03c_import_records` legacy DB upgrade 到 P2 head 成功，既有 Character / Draft 不 silent loss。
...
5. Standalone：Create / Import / Sheet / Level Up / Export 仍可用，而且沒有 Room UI / router / dependency。
...
P2 不做帳號 ownership，但需有可解釋的 Room scope。

最低產品意圖：

| 操作 | Room Member | DM | Owner |
|---|---:|---:|---:|
| Enter Room / view allowed Room data | ✅ | ✅ | ✅ |
| Create / Import Character / Draft | ✅ | ✅ | ✅ |
| Export Room Character | ✅ | ✅ | ✅ |
| 使用 Builder / Level Up | ✅ | ✅ | ✅ |
| Reversible Character archive / restore | ✅ | ✅ | ✅ |
| Permanent individual Character delete（且無 history reference） | ❌ | ✅ | ✅ |
| Create / manage Campaign / Roster | ❌ | ✅ | ✅ |
| Configure Lobby / Seats / Start / End Session | ❌ | ✅ | ✅ |
...
Session active 後，Player 對 live Character State 的實際操作權還必須受 Seat / Controller 約束；完整 GameAction permission會在 P3+ 擴充，但 P2 不可留下「知道另一角色 UUID就能 patch state」的 Web bypass。
