...
- DM Key：取得 DM 管理目前桌面／Roster／Lobby 與桌內裁定所需 authority；**不代表自動成為某場 Session 的 DM Controller，也不能自行把自己 assign 到 DM Seat。**
- Owner Key：管理 Room lifecycle、keys、Campaign lifecycle、**DM assignment** 與 destructive Room action；**Owner authority 也不自動等於本場 DM Controller**。
...
14. P2-A 導入 multiple heads 後，Web開發啟動、Docker、CI / Actions、正式文件、launcher與**既有可執行 migration tests**全部改用正確 branch-aware target；不得留下會因multiple heads直接失敗的裸 `head` contract。
15. M03 SQLite schema parity已收斂成explicit Character table allowlist；即使同pytest process先import Room tables也不受影響。
16. `P2 Non-E2E`專屬 workflow已存在並提供真 PostgreSQL migration gate；其他歷史 workflow綠燈不可替代。
17. P2-A 不建立 Campaign、Seat、Session business logic。
...
- **正常升級路徑以「P2-A 已經建立至少一個 Room」為主**：Owner 進入想接收舊資料的 target Room，在該 Room 的 `Characters` / Room Character Workshop 看到 owner-only `Legacy Character Data` migration card；只先顯示 unscoped Character / Draft 數量，不向一般 member暴露 global identities。
- migration card 提供 `Claim Legacy Character Data`；確認 modal 必須顯示 target Room 名稱、Character / Draft count，並說明這是一次性把仍 unscoped 的舊資料歸入此 Room，不會搬動已 scoped object。
- 只有「P2-B 啟用時整個 server 尚無任何 Room」才允許第一個 Owner 建 Room 時自動 claim，這是 bootstrap compatibility edge case，不是主要 UX。
...
6. `規格企劃.md` 的 `Campaign Level / Rules` 仍是產品方向，但 **P2 不新增 generic Campaign Rules blob，也不持久化 `Leveling` / `Diagonal` 欄位**：`Leveling = Milestone` 在目前只有單一升級模式時維持全產品基線；`Diagonal = 5/10 alternating` 到 P5 Tactical Combat 真正需要空間規則時交付。未來若出現第二種 leveling mode，再由對應 Phase 把它提升成 Campaign setting。
...
10. Owner / DM authority可配置目前 Campaign的 Roster與**Player / Spectator Seats**；一般 member只能操作分配給自己的 Human Player Seat，不得改他人 Seat。
...
- caller是**由Owner事前assign到該DM Seat的Human controller**，且具有 `dm` 或 `owner` Room authority；僅持DM Key但未被assign不能Start，Owner但未被assign也不能Start。
- 建立 Session identity。
- 保存本場 participants / seats；Seat歷史reference不能靠刪Seat變成null洞。
...
- 更新 Session status。
- 保存 end metadata / participants / Seat historical references。
...
6. Web：target Room Import**在P2-A exporter改版前已封存的真實legacy `unstable` fixture** → Preview → Import → Export v1。
...
- Campaign Merge。
- **Generic Campaign Rules setting / rules JSON blob；P2 不新增 `leveling_mode` / `diagonal_rule` 等欄位。`Leveling=Milestone` 先維持現有基線，`Diagonal=5/10 alternating` 由 P5 Tactical Combat 實作。**
...
- DM assignment與active Session current DM Controller已分離：Owner做Start前assignment，Session開始後controller固定。
- Session歷史Seat identity不因Seat cleanup消失。
- 同 Character active-session collision 已被阻止。
- Standalone 仍只包含 Character distribution。
- Character JSON v1 已鎖定，且legacy M03 fixture有真實 backward-compat evidence。
