...
`app.main` 與 `app.standalone` 是不同 entry point；M03 已建立 import graph gate，standalone 不得 reach Room / Campaign / Session / Seat。
...
如果 P2 直接把 Room / Campaign / Session tables接在單一 Alembic head 後面，standalone SQLite 也會長出 multiplayer schema。即使 frontend 不顯示 Room，這仍破壞 standalone「只有 Character distribution」的邊界，也會讓未來 M01 Character schema migration 無法與 Web multiplayer migration獨立演進。
...
`0009a_character_track_marker` 可是 no-op marker；目的不是造 table，而是給 standalone 一個永久可追的 shared-character branch label。
...
rooms
├─ id UUID PK
├─ code string UNIQUE         # human-enterable Room code
├─ name string
...
```text
LegacyM03CharacterExport   # exact unstable envelope contract
CharacterExportV1         # locked public contract
...
3. enforce current Room/Seat scope。
...
Web直接輸入 legacy global `/characters`：不得列出 global data；可以 redirect首頁或顯示 `room_required`，但不能當 bypass。
...
- Draft且從未建立 Session的 Campaign可 hard delete（Owner）。
...
Campaign create / archive / select active campaign屬 Owner管理。DM可管理目前 Campaign的 Roster與 Lobby，但不因持有 DM Key就取得 Room lifecycle ownership。
...
- Owner：Room / Campaign lifecycle與全部管理；若不是本場 DM controller，不因此獲得「End Session」的桌上裁定權。
- DM authority：可管理目前 Campaign Roster / Seats / Lobby；Start Session要求實際綁定 DM Seat。
- Member：可使用 Room Character Workspace，並可 claim / operate被分配給自己的 Human Player Seat；不能改 Campaign lifecycle / 他人 Seat。
...
3. caller具有 DM authority且是本場 human DM Seat controller（P2）。
...
1. DM配置／選一個尚未參與本 Session的 Player Seat（或建立新 Seat）。
...
`End Session`：只有 current DM controller可執行；Owner authority本身不是 substitute DM。

`Abandon Session`：current DM或 Owner可執行，用於誤開／DM不再回來。
...
寫入流程除 Room membership外，若 Character有 active Session lease：

```text
DM controller / authorized DM
OR
Human Player Seat controller for that leased participant
```

才可做 live Current State mutation。

如果沒有 active Session lease，Room member仍可依朋友間協調使用 Character Workshop / Sheet；不建立永久 Character ownership。
...
- Human DM可依既有產品規格修改 Build。
- 其他一般 member不能因知道 UUID修改正在別人 Seat中的 Character。
...
測試必須同時在 PostgreSQL與必要 SQLite-neutral core test證明沒有 FK ordering / orphan問題；standalone本身不執行此 service。
...
5. web migration target使用 `heads`。
6. future shared Character migration有明確 `character` branch落點。
7. Web global Character API沒有 scope bypass。
8. Room Character / Draft association transaction atomic。
9. cross-room UUID access default deny。
10. Roster不複製 CharacterState。
11. active Character lease在 concurrent Session下由 DB invariant保護。
12. Session End / Abandon不 mutate CharacterState。
13. DM Controller freeze由 Server資料 enforce，不靠 UI。
14. Character JSON v1不含 Room資料且 legacy unstable importer仍存在。
15. Room Hard Delete無 P2 / Character orphan。
16. P2 user-visible copy雙語完整。
