...
### 3.4 P2-A 必須同步修改所有實際 Web migration entry

Branch split 一旦提交，單數 `head` 會變 ambiguous。P2-A 同一個 code Subphase 必須同步更新所有真正會啟動／驗證 Web 的地方，例如：

- `docker-compose.yml` server startup command。
- README 的 Web migration 指令。
- Web / backend CI workflow 或 helper script中任何 Alembic upgrade target。
- dev / release script若有 Web DB migration command。

原則：

```text
Web → heads
Standalone → character@head
```

不要保留一條「平常其實會被人照著跑」的 Web `upgrade head`。歷史 closeout文件中的舊命令可以保留，因為它描述當時事實，不是現行操作指南。
...
Room Password是人類可能選弱字串，必須走 slow password KDF。第一版可使用 stdlib `hashlib.scrypt` + per-room random salt，避免為 MVP只為 password新增大型 auth framework。
...
不要用「查不到 UUID」與「沒權限」混成會洩漏跨 Room存在性的錯誤；跨 Room resource對一般 caller可統一 404-style resource-not-found presentation。

### 5.6 Presence

P2 不建 WebSocket event bus。

Human `Connected / Offline` 可先由：

```text
room_access_sessions.last_seen_at
+ lightweight HTTP heartbeat
```

推導。這只做 Lobby presence，不拿來決定 GameAction ordering。
...
Lobby Connected / Offline由 RoomAccessSession heartbeat推導；不把 Seat row delete當掉線。

P2不使用 arrival timestamp決定遊戲順序。

### 9.7 Permission

- Owner：Room / Campaign lifecycle與管理；若不是本場 DM Controller，不因此獲得 Start / End / Late Join 的桌上裁定權。
- DM authority：可管理目前 Campaign Roster / Seats / Lobby；Start Session仍要求 caller實際綁定該 DM Seat並成為固定 DM Controller。
...
├─ seat_id nullable FK campaign_seats
├─ role_snapshot
...
3. caller具有 DM authority、是該 Human DM Seat controller，且本次 Start將這個 identity固定成 Session DM Controller；Owner authority本身不能代替此條件。
...
5. delete P2 Session / Seat / Roster / Campaign rows（可由 Room FKs cascade，仍要驗 orphan）。
...
5. Web migration target使用 `heads`；Docker / README / CI / active scripts沒有 stale Web `upgrade head`。
6. future shared Character migration有明確 `character` branch落點；Character ancestry永遠不含 web revision。
7. Web revision若依賴 Character revision，只使用單向 dependency，不 merge branches。
8. Web global Character API沒有 scope bypass。
9. Room Character / Draft association transaction atomic。
10. cross-room UUID access default deny。
11. Roster不複製 CharacterState。
12. active Character lease在 concurrent Session下由 DB invariant保護。
13. Session End / Abandon不 mutate CharacterState。
14. Start / Late Join / End只接受 current Session DM Controller所需 scope；Owner只額外允許 Abandon，不是 substitute DM。
15. Character JSON v1不含 Room資料且 legacy unstable importer仍存在。
16. Room Hard Delete無 P2 / Character orphan。
17. P2 user-visible copy雙語完整。
