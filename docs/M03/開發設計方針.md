  ...
  standalone-build:
    ...
    steps:
      ...
      - name: Build standalone
        shell: cmd
        run: scripts\build-standalone.cmd --version ${{ steps.version.outputs.value }}
      - name: Smoke test
        shell: cmd
        run: .standalone-venv\Scripts\python.exe scripts\smoke_standalone.py dist\adventure-table-standalone --timeout 30
      - uses: actions/upload-artifact@v4
        with:
          name: adventure-table-standalone-${{ steps.version.outputs.value }}
          path: dist/adventure-table-standalone-${{ steps.version.outputs.value }}.zip
          if-no-files-found: error
```

**關鍵**：
- **沒有 release job，也沒有 tag `v*` 觸發。** 發版一律在本機跑 `scripts\build-standalone.cmd --version <版本>`；理由見實作規格 F.2（repo 將轉 private，private repo 的 Release asset 無匿名下載連結）。`permissions` 因此維持 `contents: read`。
- Smoke 用 build 出來的 `.standalone-venv` 直接跑，不另外裝 Python 相依。
- Version 一律由 commit sha 推導，因為沒有 tag 觸發路徑。
- Release notes 模板 `docs/M03/release-notes-template.md` 於 M03-F 內建立，內含 JSON schema `unstable` 說明，供本機發版時複製使用。
