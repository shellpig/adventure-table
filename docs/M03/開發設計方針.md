...
apps/server/
  app/
    ...
    persistence/
      ...
      character_imports.py           # M03-C 新增（import records + XOR CheckConstraint）
...
class Settings(BaseSettings):
    ...
    enabled_content_packs: tuple[str, ...] = Field(
...
**關鍵**：**不** 啟用 `env_prefix`，維持 Docker `DATABASE_URL` 直接讀取。新欄位靠 `AliasChoices` 明列環境變數名。`enabled_content_packs` 的 `field_validator` 把 comma string 轉為 tuple，同時接受 iterable。
...
character_import_records
  ...
  CHECK ((character_id IS NULL) != (draft_id IS NULL))       # XOR
```

- `character_id` / `draft_id` 二選一 nullable：character 落地時 draft_id NULL，反之亦然。CheckConstraint 保證。
...
- `ADVENTURE_TABLE_ENABLED_CONTENT_PACKS`：逗號分隔覆寫 pack 集合，經 `field_validator` parse。
...
<next revision> M03-C character_import_records（含 XOR CheckConstraint）
```
