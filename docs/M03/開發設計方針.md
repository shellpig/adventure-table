...
def resolve_database_url() -> str:
    ...
    if path:
        ...
        return f"sqlite+pysqlite:///{abs_path.as_posix()}"
...
def _print_banner(port: int) -> None:
    ...
    print(f"  Database: {os.environ.get('ADVENTURE_TABLE_DATABASE_PATH') or '<settings>'}")
...
- `ADVENTURE_TABLE_DATABASE_PATH`（standalone）：覆寫 SQLite 檔位置；透過 `resolve_database_url()` 決定最終 URL。
