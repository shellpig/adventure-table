...
> 狀態：**M03 未開工**。三份文件於 codex 兩輪 review 後重寫，等 M01 Full Closeout 後才進入 M03 coding。
...
def _resolve_default_database_path() -> Path:
    """雙擊 EXE 的預設值：執行檔同層 adventure-table.sqlite3。

    frozen 時 sys.executable 的父目錄即產物根目錄；非 frozen（dev 直接跑
    launcher）時退回 CWD。兩者都不是 Postgres。
    """
    if getattr(sys, "frozen", False):
        return (Path(sys.executable).resolve().parent / STANDALONE_DB_FILENAME)
    return (Path.cwd() / STANDALONE_DB_FILENAME).resolve()


def main() -> int:
    # 第一步就把資料檔路徑釘死，之後所有 resolver / alembic / engine 都讀同一個值。
    # setdefault：外部已設 ADVENTURE_TABLE_DATABASE_PATH 時尊重覆寫。
    os.environ.setdefault(
        "ADVENTURE_TABLE_DATABASE_PATH", str(_resolve_default_database_path())
    )
...
- `ADVENTURE_TABLE_DATABASE_PATH`（standalone）：覆寫 SQLite 檔位置。未設時 launcher 於 `main()` 最前面以 `<exe_dir>/adventure-table.sqlite3`（非 frozen 時 `<cwd>/…`）`setdefault` 寫入；最終 URL 由 `resolve_database_path()` → `resolve_database_url()` 決定。
