...
4. **Seat private event 的合法讀者是指定 Seat 的 current controller 加上本場 current DM Controller。** P3 只有 Whisper DM、不做 player-to-player private message，DM 是每一條私密流量的另一端；不得讓 Seat private event 變成連 DM 都讀不到的孤兒 event。
5. **私密 event 的可見範圍以 Seat controller epoch 為界：一個 controller 只能讀到自己這個 generation 開始之後產生的 Seat private / actor-private event。** Human → AI 交接、AI rotate 與 administrative reassignment 後，接手者讀不到交接前的 Whisper DM 與其他 Seat private 歷史。**public event、Stage、current pending action / roll 與 Character Current State 不受此限**，接手者仍必須拿到完整桌面現況；這與 Late Join 拿得到 current context、拿不到舊 private event 是同一條規則。current DM 不受此限。
6. 上一條的唯一例外是 self-service `Take Back Control`：發起者就是當初執行 `Let AI Control` 的同一個 Human access session（見 P3-D），該身分在交接前已合法看過那段私密歷史，因此拿回控制權後可見範圍還原到交接前，不重新遮蔽。
7. caller 只能取得自己有權看到的 event；直接猜 event id / sequence 不得繞過 visibility 或上述 controller epoch 邊界。
8. browser 初次進 Session 先取得 Resume / current runtime truth，之後用 incremental event feed 更新；不得高頻重新抓完整 `SessionResumeService` 當即時同步方案。
9. reconnect 可以帶 `after_seq` / 等價 cursor 只補缺的 event；cursor 太舊或資料不可用時可以安全 fallback 到完整 current-state resume。
10. server restart 後 event order、runtime revision、pending truth 與各 Seat 的 controller epoch / private visibility floor 不重置、不倒退。
11. active Session 的新 runtime write 必須確認 caller 仍屬於該 Session 的合法 controller / participant scope。
12. event wait / long-poll 必須使用不佔住同步 worker 的 async request path；等待期間不得持有 SQLAlchemy connection / transaction。Process-local notifier只能負責 wake-up，wake / timeout後仍以短 DB recheck + durable cursor為真。
13. 多個 idle waiter存在時，一般 DB-backed Session/event read/write仍必須可完成；client cancellation / timeout必須釋放 waiter resource，不能靠把 timeout縮短來掩蓋 starvation。
14. P3-A 不建立 Chat、Roll、AI token 等上層產品行為，只交付它們可依賴的 substrate 與最小 diagnostics；controller epoch / private floor 的實際寫入方由 P3-D 交付，P3-A 只保證 projection 會套用它。
15. P3-A 不把 P7 的完整 Timeline UX、跨 Session history search、Snapshot / Restore 搬進來。
16. 若 Session 頁需要提高同步頻率，必須先解除 P2 closeout 記錄的 Resume / Lobby N+1 高頻風險；不能用更短輪詢週期放大既有 N+1。
17. Web capability 新增 table-runtime / event 能力時，Standalone 同能力必須維持 false / unavailable。
...
13. Whisper DM 只對 sender Seat 的 current controller 與 current DM 可見；其他 Player / AI Player 的 event feed 根本不含該內容，且依 P3-A 的 controller epoch 邊界，接手該 Seat 的新 controller 也讀不到交接前的 whisper。
...
5. **未綁 Session 的 pre-session AI DM grant 必須有有限有效期，且不是永久 DM bearer credential。** 在 `session_id` 尚未綁定時，它只能用於取得自己被指派 DM Seat 的最小 pre-session context與 Start Session所需流程；不能讀 active Session private event、Chat/Narration、Roll、Character State或其他 gameplay runtime。
...
17. **controller epoch 前進時，必須在同一 transaction 維護該 Seat 的 private visibility floor（見 P3-A）。** `Let AI Control`、AI rotate / regrant 與 administrative reassignment 設新 floor，使接手者讀不到交接前的 Seat private event；self-service `Take Back Control` 還原 handoff 當下保存的舊 floor，讓原玩家拿回角色後仍看得到自己送出的 whisper。floor 只影響 private event 讀取範圍，不影響 public event、Stage、pending action / roll 與 Character Current State，也不參與授權判斷。
18. DM Controller在一場 Session開始後仍固定，不支援 Human DM ↔ AI DM中途交接。
...
20. P3 必須讓 **AI DM 可以成為新 Session 的起始 DM Controller**：Owner在 Lobby事前把 DM Seat配置成 AI並產生 pre-session scoped credential；AI join後可以依同一 Session Start policy啟動該場。Start transaction把仍有效且未過期的 grant綁到新 Session，之後該 AI controller identity固定到 End / Abandon。
21. Seat / Session / Participant persistence必須能明確保存 AI controller grant與必要的 generation/epoch snapshot，並用 DB constraint區分 Human binding、AI binding、None；不能只靠 `controller_kind='ai'` 加上所有既有 Human access-session欄位為 NULL 來代表「某個 AI 正在控制」。
22. **Session End / Abandon 必須撤銷所有仍可操作該 Session 的 session-scoped AI grant / token generation。** End / Abandon後舊 token不能繼續 read private events、wait、Chat、Roll或 state write；下一場需要新的合法 grant。
23. Session End / Abandon 的 token revoke必須和 Session lifecycle transition、pending close/freeze、final event與 P2 lease release處在同一可證明的 atomic lifecycle boundary；不能先 End成功再 best-effort revoke。
24. AI DM disconnect不會自動 End Session，也不讓 Owner或另一 AI token接管；Owner仍只有既有 Abandon escape hatch。
25. revoke / rotate / handoff / Take Back / administrative reassignment在 DB transaction與 concurrency下安全；兩個同時變更 controller的請求不能留下兩個有效 generation或讓 stale grant繼續通過授權。
26. Human / AI reconnect後都從 Server取回目前 controller scope、Temporary Handoff Instruction（若仍有效）與可見 pending state，不依賴舊 conversation memory。
27. P3-D 只完成 controller與 credential，不在此 Subphase 宣稱 MCP tool surface已交付；正式 external AI 使用在 P3-E驗收。
...
8. AI DM可由Owner事前配置、使用**有限 TTL**的 pre-session scoped credential加入並Start一場新的 Session；未綁定時只能執行最小 pre-session/Start能力，Seat/controller/Campaign lifecycle變化或到期會使其失效，Session中不得被Human/另一AI接管。
...
11. **private visibility floor 有整合證據**：接手的 AI 讀不到 handoff 前的 Whisper DM 與 Seat private 歷史，卻仍取得完整 public 桌面現況與 pending state；self-service Take Back 後原玩家重新看得到交接前自己的 private 歷史；administrative reassignment 接手的新 Human 則看不到。current DM 全程可讀該 Seat 的 private event。
12. 真 external MCP client可取得 context、送 action、wait event、完成 formal check；不是只用 direct Python function或 mock client。
13. browser reload、server restart之後 Stage、committed Chat / Action、PendingAction、Pending Roll、resolved Roll與有效 controller scope可正確恢復。
14. Session End不修改 Character Current State；P2 invariant持續成立。
15. Cross-Room / cross-Session / cross-Seat authorization matrix全綠，包含猜 UUID、猜 sequence、舊 token、revoked token、expired pre-session token與 stale controller generation。
16. event order / duplicate submit / controller handoff race在真 PostgreSQL有 concurrency evidence。
17. event wait在多個並發 idle waiter下不得餓死一般 DB-backed request；waiter timeout / cancellation後 resource會釋放，且等待期間不持有 DB connection / transaction。
18. P3 新增 multiplayer tables / modules仍被 M03 standalone boundary gate禁止進 standalone schema / import graph。
19. Web / Standalone capability雙向測試完整；P3頁面與MCP endpoint不會在 standalone channel誤開。
20. P3 user-facing UI / errors / accessibility text都有 `zh-TW` / `en`，Session頁加入整頁 bilingual crawl / overflow / raw-key gate。
21. Full existing P0 / P1 / M01 / M02 / M03 / P2 regression不因P3 route、state write或permission wrapper而退化，尤其 P2 Session lifecycle / late join / live character scope / controller persistence tests在 caller abstraction migration後仍全綠。
22. P3 closeout evidence記錄 exact SHA、backend/frontend測試、PostgreSQL migration/concurrency、Playwright、standalone frozen smoke與真 MCP client smoke。
23. P3 closeout不得宣稱已完成 P4 Combat、P6 Adventure Runtime / high-level `resolve_action()`或P7 Timeline / Snapshot；文件明列仍然存在的產品邊界與已知限制，包含「P2 Room access session不是跨裝置持久 Human identity」。
...
- **跨裝置／跨 browser 的永久 Human account identity。** P3 `Take Back Control`只能可信地驗證原 `Let AI Control` 的同一 Room access session；若該 access session遺失，改走 Owner/DM administrative Seat reassignment，不用 display name或同 Room member冒充同一人。
