...
- Owner Abandon escape hatch仍成立，但Owner不是current DM時不能End / Late Join /改Stage / proxy。（2026-09-20 M05-A 修訂：Owner 對 **AI DM** 的 active Session 另可正常 End；Human DM 場與其他 current-DM-only 操作不變。M05-A 測試見 `docs/M05/測試指南.md`。）
- stable error code。
...
- Owner可Abandon不會讓Owner通過current-DM-only Stage / Request Check / End / Late Join / proxy path。（2026-09-20 M05-A 修訂：此 journey 的 AI DM 場，Owner 另可正常 End；Stage / Request Check / Late Join / proxy 仍不通過。）
...
14. `post_action`後Human browser/event API看到同一canonical action。
15. Human DM Request Check後AI Player `get_pending_events`看到pending。
16. AI `roll_pending`完成後Human UI看到同一result。
...
- Human controller Seat with controller_epoch。
...
26. `CapabilityFlags` 是否沿用既有 `ai_actor`；`combat` / `timeline` 是否仍false；新增flag是否backend/frontend同步。
