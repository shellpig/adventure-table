...
- Player Human→AI handoff grant：`session_id` 一開始就有值，`handoff_return_access_session_id` **必填**且等於執行 `Let AI Control` 的 Human access session；`pre_session_expires_at=NULL`。
- pre-session AI DM grant：`session_id=NULL`、`handoff_return_access_session_id=NULL`、`pre_session_expires_at` **必填且必須是有限未來時間**。TTL長度可由 server config決定，但不允許「永不到期」作為預設或 production-like gate。
...
`End Session`由 fixed current DM actor執行，因此 Human DM / AI DM都可依同一 policy End。（2026-09-20 M05-A 修訂：唯一新增例外是 Human Room Owner 對 **AI DM** 的 active Session 可正常 End，走與 Owner Abandon 相同的 Room-management 分支；Late Join／Stage／proxy／Request Check 等 current-DM-only 權限不因此放寬。見 `docs/M05/開發設計方針.md` §3.1。）
...
- `Let AI Control`：Seat epoch increment + token revoke/mint + return access identity + Temporary Instruction context + Seat binding + controller changed event。
- `Take Back Control`：origin identity recheck + AI grant revoke + Seat epoch increment + Human binding restore + controller changed event。
- administrative Player Seat reassignment away from AI：admin auth + AI grant revoke + Seat epoch increment + new Human/None binding + audit event。
