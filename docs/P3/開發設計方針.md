...
- SEAT_PRIVATE：指定 Seat controller **+ current DM**。

**current DM 可讀 `SEAT_PRIVATE` 是已拍板的產品決定，不是實作選項。** 理由：P3 只有 Whisper DM，`規格企劃.md` 第十二章明確不做 player-to-player private message，因此桌上每一條私密流量的另一端本來就是 DM；若 DM 讀不到，`SEAT_PRIVATE` 會產生沒有任何合法讀者的孤兒 event。三份 P3 文件不得再把這條寫成「依產品行為」「依contract」之類的待定敘述。

不得讓 caller query傳 `include_dm_only=true` 自己選 audience。

### 5.3.1 private visibility floor（controller epoch 邊界）

audience 綁 **Seat**，但 Seat 的 controller 會在一場 Session 中換人（Human → AI → Human、rotate、administrative reassignment）。若只用 Seat 判斷，接手的 AI 會讀到真人玩家在接手前送出的 Whisper DM 與其他 Seat-private 歷史。P3 固定：

> **私密 event 的可見範圍以 controller epoch 為界。一個 controller 只能讀到自己這個 generation 開始之後產生的私密 event。**

規則：

- 每個 Seat 持有一個 **persisted private visibility floor**（例如 `campaign_seats.private_floor_seq`），語意是「目前這個 controller 可讀的最小私密 event seq」。
- floor 只約束 `SEAT_PRIVATE` 與以該 Seat 為 actor/subject 的 `ACTOR_AND_DM`。**public event、Stage、current pending action / roll、Character Current State 一律不受 floor 影響**：接手者必須拿到完整的桌面現況才能繼續玩，這與 Late Join 拿得到 current context、拿不到舊 private event 是同一條規則。
- `controller_epoch` 增加時，在同一 transaction 把 floor 設為該 epoch 起始（建議直接用本次 controller-changed event 的 `seq`）。適用於 Human → AI、AI rotate / regrant、以及 administrative reassignment。
- **例外：self-service `Take Back Control` 還原 handoff 前的 floor，不設新 floor。** 依 §8.5「Take Back Control self-service identity」，self-service Take Back 只授權給 `handoff_return_access_session_id`，也就是當初執行 `Let AI Control` 的同一個 access session；該 identity 在 handoff 前就已合法看過那些 private event，重新遮蔽它沒有保護對象，只會讓玩家拿回角色後看不到自己送出的 whisper。因此 grant 必須保存 handoff 當下的 floor 以便還原。
- current DM 不受 floor 影響。DM Controller 整場固定，沒有 epoch 轉換問題；AI DM reconnect 使用同一 grant / generation，也不觸發 floor。
- floor 由 Server 在 query / projection 階段套用，和其他 audience filter 同一層；不得送出後再由 client 自行截斷。
...
campaign_seats
  ...
  private_floor_seq bigint NOT NULL  # §5.3.1 private visibility floor, default 0
...
`campaign_seats.private_floor_seq` 是 §5.3.1 的 private visibility floor，與 `controller_epoch` 在同一 transaction 一起維護：epoch 前進時 floor 一併更新（self-service Take Back 為還原、其餘為設新值）。它只影響 private event 讀取範圍，不參與 authorization 判斷，因此不進 binding CHECK。
...
handoff_return_private_floor_seq nullable
...
- Player Human→AI handoff grant：`session_id` 一開始就有值，`handoff_return_access_session_id` **必填**且等於執行 `Let AI Control` 的 Human access session；`handoff_return_private_floor_seq` **必填**且等於 handoff 當下 Seat 的 private visibility floor（供 §5.3.1 的 self-service Take Back 還原）；`pre_session_expires_at=NULL`。
- pre-session AI DM grant：`session_id=NULL`、`handoff_return_access_session_id=NULL`、`handoff_return_private_floor_seq=NULL`（DM 依 §5.3.1 不受 floor 約束）、`pre_session_expires_at` **必填且必須是有限未來時間**。TTL長度可由 server config決定，但不允許「永不到期」作為預設或 production-like gate。
- grant `generation` 一經mint不可改；controller epoch要前進就mint新grant，不更新舊grant generation冒充新controller。
...
- **Player Human → AI handoff發生在 active Session**：lock Seat、先增加 `controller_epoch`，mint `generation=controller_epoch` 的 grant並直接綁 `session_id`，再把 Seat current AI grant binding指向它。
- **AI DM在Start前需要credential**：Owner配置 DM Seat AI時同樣先增加 Seat epoch，mint `generation=controller_epoch`、`session_id=NULL`、有限 `pre_session_expires_at` 的 pre-session DM grant，Seat current grant binding指向它。
- 一個已綁 Session 的 grant不能重綁到另一 Session。
- Start transaction只能把**仍 active、未過期、仍是 DM Seat current grant、generation仍等於Seat epoch**的 pre-session grant綁到新 Session。
...
`session_id=NULL` 時，它**不是 current Session DM**。允許面只包含：

- MCP transport metadata / discovery / tool list等不含 gameplay state的protocol操作。
- 讀取**自己** DM Seat、Campaign與 Start readiness所需的最小 pre-session context。
- `Start Session`。

明確禁止：

- `get_session_context` / Session event read / wait。
- Chat / Narration / Stage / Request Check / Roll / Character State write。
- Late Join / End / Abandon。
- 任何 Owner / Room destructive management。
- 其他 Seat private context。

#### Unbound grant revoke / expiry lifecycle

pre-session DM grant在以下事件發生時立即失去授權能力；有對應 mutation transaction時應在同一 UoW revoke：

- TTL 到期。Authorization 必須把 expired視為 revoked，即使沒有背景job把 `status`欄位即時改掉；可在下次讀寫時 lazy persist revoke，但安全不能依賴 cleanup job。
- DM Seat controller改成 Human / None。
- 同一 DM Seat rotate / regrant到另一 AI grant或任何使 Seat epoch前進的 controller mutation。
- DM Seat archive / delete。
- Campaign從 Start-eligible active狀態離開（例如轉 `completed`、`archived`，或現行允許的其他非active狀態）。
- Room `active_campaign_id` 切離該 Campaign時，若該 grant尚未綁 Session，應revoke；重新選回Campaign後重新mint，避免 dormant DM bearer長期留存。
- Room / Campaign hard delete（可由 cascade刪除grant，但 token必須立刻不可解析／授權）。
- Owner explicit revoke。
...
7. mint new **session-bound** grant，`generation = seat.controller_epoch`，保存 `handoff_return_access_session_id = caller.access_session_id`、`handoff_return_private_floor_seq = seat.private_floor_seq`（handoff 前的舊 floor）+ optional Temporary Instruction。
8. switch Seat controller to AI並bind該grant id。
9. append controller-changed event。
10. 依 §5.3.1 把 `seat.private_floor_seq` 設為該 controller-changed event 的 `seq`，使接手的 AI 讀不到 handoff 前的 Seat-private 歷史（Instruction本身不必做public event）。
11. commit。
...
P2目前沒有帳號級 persistent Human identity；Room access session就是可驗證的 Human credential。因此 P3-D **不得**用 `display_name`、同一 Room member、Room password、IP/browser fingerprint等猜測「這還是原玩家」。
...
6. **還原** `seat.private_floor_seq = grant.handoff_return_private_floor_seq`。依 §5.3.1，self-service Take Back 的 caller 就是 handoff 前那個已合法看過該段私密歷史的 access session，因此這裡不設新 floor。
7. append controller-changed audit event。
8. commit。

原 handoff Human若換裝置／清 localStorage而拿到新的 Room access session，即使 display name相同，也**不能**走 self-service Take Back。這不是 P3 可安全推導的同一個人。
...
- 保留 admin acting identity + subject Seat + new Human access session audit。
- lock Seat。
- revoke current AI grant。
- increment `controller_epoch`。
- set `controller_kind=human` + new `controller_access_session_id`。
- 依 §5.3.1 把 `private_floor_seq` 設為本次 controller-changed event 的 `seq`。這條 path 接手的是**無法證明為原玩家**的另一個 access session，因此必須設新 floor，不得比照 self-service Take Back 還原舊 floor。
- 保留 Session participant / Active Character snapshot不變。
- 同 transaction commit controller-changed event。
...
- caller必須就是被assign的current grant identity，且grant未過期、Seat epoch仍匹配。
- Campaign必須仍是 Start-eligible 的目前 active Campaign。
- Human DM仍需既有Room authority；AI DM不取得Owner key，但成功Start後grant有該Session的DM gameplay scope。
...
- 不revoke已綁 active Session的token。
- 不End Session。
...
AI DM：**只有 grant已成功綁 active Session後**才取得DM-visible table state、pending check info、participant summaries。Pre-session AI DM token只能用 §8.3 / §9.3 的最小pre-session context。
...
- `Let AI Control`：Seat epoch increment + token revoke/mint + return access identity + saved return private floor + Temporary Instruction context + Seat binding + controller changed event + **new private floor set**。
- `Take Back Control`：origin identity recheck + AI grant revoke + Seat epoch increment + Human binding restore + **private floor restore** + controller changed event。
- administrative Player Seat reassignment away from AI：admin auth + AI grant revoke + Seat epoch increment + new Human/None binding + **new private floor set** + audit event。
- pre-session DM grant create/rotate：old grant revoke + Seat epoch increment + finite-TTL new grant + DM Seat current binding。
- DM Seat controller change/archive/delete、Campaign離開Start-eligible狀態或Room active Campaign切離時，與相關**未綁Session** DM grants revoke同一 lifecycle mutation boundary。
- AI DM Start + pre-session grant current-binding/TTL recheck + grant bind + Session fixed controller snapshot。
...
1. audience filter與 §5.3.1 private visibility floor都在Server projection同一層套用；不得任何一項只在frontend或context builder執行。
...
13. `Take Back Control`只能由grant保存的原 handoff Human access session self-service；其他member、相同display name、新access session一律不能冒充。遺失原session時只走Owner/DM administrative reassignment。
14. Temporary Handoff Instruction不得進public event、Character export或Campaign permanent fact；失效後不再出現在AI context。
15. DM proxy只有**fixed current DM actor**可用；持DM Key但不是本場DM Controller不得proxy。
...
- pre-session AI DM finite-TTL / restricted-surface / automatic lifecycle revoke evidence。
- Take Back origin-Human authorization + cross-device/admin-reassignment evidence。
- Seat controller-epoch / grant-generation stale-token evidence。
