...
> A～N 是截至 2026-09-06 已完成的 baseline；M01 現為**長期保持 open 的 Character Content Expansion / Maintenance track**。本文件定義未來 M01-O+ 的共通驗證原則，以及可在重大 regression / P Phase closeout重用的 cumulative Character baseline。Subphase名稱與順序必須與 `實作規格.md`、`開發設計方針.md` 完全一致。
...
未來 M01-O+若改到上述任一類，優先重跑既有對應 focused suite，而不是複製第二套測試。
