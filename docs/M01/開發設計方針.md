...
> A～N 是截至 2026-09-06 已完成的 Character Content baseline；M01 現為**長期保持 open 的 Character Content Expansion / Maintenance track**。本文件定義未來 M01-O+ 必須延續的工程契約，以及已完成 baseline 不可破壞的邊界。完成後必須為真以 `實作規格.md` 為準；測試證據以 `測試指南.md` 為準。
...
**下一個新的 M01 工作從 M01-O開始。** 不預留任何字母給 Full M01 Integration & Closeout。M01可以長期保持 open，同時 P2、P3… 正常往前；每個新 M01 Subphase自己設計、自己 closeout。
