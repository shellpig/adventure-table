...
- `race in {human, half-elf, half-orc}`
- `race == dwarf OR size == small`

Half-Elf variant、Tiefling bloodline / variant等既有 replacement composition不得因此失去其 canonical base ancestry資格。若現有 Build只保存 StableKey而沒有 ancestry tag，應在 shared content facts/resolver補最小 normalization，不在每個 Feat資料內硬列所有 variant StableKey。
...
Squat Nimbleness的 Acrobatics / Athletics使用同一 skill proficiency primitive；Chef / Poisoner工具熟練亦使用同一 tool proficiency grant。
...
- Fey Teleportation：Misty Step 1/SR or LR；INT。
- Wood Elf Magic：1 Druid cantrip + Longstrider / Pass without Trace 1/LR；WIS。
- Artificer Initiate：1 Artificer cantrip + 1 Artificer level-1 spell；free cast 1/LR並可用自身 spell slots；INT。
...
- Telepathic：Detect Thoughts free cast 1/LR並可用 spell slots；casting ability綁定所選 INT/WIS/CHA。
