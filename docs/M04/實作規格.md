...
- `get_session_context`（active）與 `start_session` 回傳加 `stage_unset: bool` 與 `next_required_action: str`，讓 AI 不必只靠 briefing 文字。
- DM 且 Stage 空 → `next_required_action = "set_stage_text"`；否則 → `"wait_for_event"`。Player 一律 `"wait_for_event"`（不引導去碰 DM-only Stage）。
- **純提示，server 不強制**：沒設 Stage 仍可 `post_narration`，守住「Optional 不得變 Mandatory」。
...
**2026-09-12 修訂（使用者拍板）**：本 Subphase 的 C.8 gate 改為**目標平台 ChatGPT Web Plus 憑 kit 進桌一次**；上列 Bearer MCP client 與純 HTTP 兩條路徑**延後**，不列為 M04-C 關門條件。理由：目標平台是 M04 的必要成功條件，其餘路徑屬相容記錄。證據見 `M04-C_CLOSEOUT.md`。
