...
- 必要時 DM 可 Undo 或 Direct Edit 修復。
...
第一頁包含：Ability Scores、Skills、Saves、Passive Perception、Features / Traits、Conditions、Temporary Effects。Roleplay / Biography 放底部折疊。
...
每次 Level Up 保留完整 immutable previous version，不是 diff。

Snapshot 包含：Class / Level、Stats、HP、AC、Skills / Saves、Features、Spells、Slots、Inventory、Currency、Conditions、Roleplay、Custom data。
...
**Spell Slot 是資源；Spell Access 是會不會／能不能準備。兩者分開。**

每個施法職業有獨立 Spellcasting Profile。Spell entry 至少知道：
...
Wizard：Spellbook + Prepared；冒險途中可 Add to Spellbook。

Cleric / Druid 類：`prepared_spells[]`，可用法術依 class / level 計算。
...
Build：Class、Base stats、Features、Spellbook、Max resources（含 Max HP、Hit Dice 總數與骰型）。

Current State：Current HP、**Temporary HP**、Current Slots、**Available Hit Dice**、Prepared Spells、Concentration、Conditions、Temporary Effects、Death Save 進度、Inspiration、Consumables。
...
- Review 顯示 Build Summary、Stats、Features、Spells、Equipment、Warnings、Non-standard Values。
...
## 九、Timeline、GameTransaction、Undo 與 Snapshot
...
Player Draft 可 Cancel。Committed GameTransaction 由 DM Undo。

Undo 不刪歷史，而是建立 Revert transaction。適合修剛剛點錯，不適合重寫半小時前的劇情。

Automatic Snapshot：Session Start、Scene Change、Combat Start、Combat End、Session End。

Manual：DM Save Point。

Rollback 後 Active State 回去，但後續 history 保留成 archived/inactive branch。UI 不做 Git。小錯用 DM Direct Edit。
