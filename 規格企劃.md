...
12. **介面支援繁體中文與英文兩個純語言模式**：使用者可在 `zh-TW` 與 `en` 間切換；目前語言下的系統 UI、user-visible rules names / descriptions、系統提示與錯誤訊息應使用同一語言，不以中英並列作正常顯示方式。語言切換是 presentation 行為，不改變遊戲狀態。
...
- 語言可在網站內一鍵切換，**不需要重新載入整個頁面**。
- 切換後留在目前頁面／工作流程；Builder 中途切換不能丟失目前 Draft 或選擇。
- 使用者選擇的語言會被目前瀏覽器記住，reload 或下次再開網站仍沿用。
- 繁中模式正常顯示繁中 system / rules presentation；English mode 正常顯示 English system / rules presentation。
- 不以 `火球術 / Fireball`、`Race：木精靈` 這類雙語混排作正常 UI；搜尋可以接受另一語言名稱，但結果仍以目前語言顯示。
- Character name、Biography、Appearance、玩家／DM 自己輸入的 Roleplay / Notes 等 **user-authored free text 不自動翻譯、不因切換語言而改寫**。
- `D&D`、`SRD`、`PHB`、骰子記法、數值、StableKey、正式來源縮寫或其他非自然語言 technical token 可按其正式形式保留，不視為混語缺陷。
- 規則 identity、角色選擇與 Game State 不因語言改變；翻譯只改 presentation。
- 第一版不做第三種語言、使用者自訂語言包、Campaign / Character 各自的語言設定，也不做 runtime LLM 即時翻譯。
...
- AI DM 主持時的任何還原功能
- 第三種介面語言、使用者自訂語言包、runtime LLM 即時翻譯（第一版只做 `zh-TW` / `en`）
...
已定案並收錄的項目包括：原 handoff 的 Party Roster / Character 加入退出（第三章）；以及 2026-08-29～2026-08-30 增補的 AI DM Build 修改限制、玩家自行扣 HP、DM 代理 Seat、Session 選角與中途不換角、DM 中途離線、Temporary HP、Hit Dice、Quick Combat 不追移動、Death Save 於 End Combat 清除、不接 LLM API、繁中／英文純語言介面、DM Notes 保留。
