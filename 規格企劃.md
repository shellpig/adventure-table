...
- Room Password：進房，取得一般 member access。
- DM Key：取得 DM authority；不等於自動成為某場 Session 的 DM Controller。
- Owner Key：管理 Room / Campaign lifecycle / Assets / DM assignment / destructive actions。
- AI：revocable scoped Seat token。
- AI 永遠不拿 Owner Key。
...
- Caller 只能做目前 Room / Role / Seat / Scope 允許的操作。
...
- Web Character 從建立／匯入後就由某個 Room workspace 管理。
- 未完成的 Builder Draft 也從建立當下就屬於 Room。
- 同一 Character instance 一次只屬於一個 Room。
- 要把角色帶到另一個 Room：Export / Import 或等價 Copy-as-new，目標 Room 產生新的 Character identity；兩邊之後各自發展。
...
- Characters。
- Builder Drafts。
- Campaigns / Rosters / Seats / Sessions。
- Maps / Adventures / Room assets（對應系統存在後）。
...
**AI DM 的 Session 由 Owner 收尾。** AI DM 通常不會自己按 End。當 active Session 的 DM Controller 是 AI，Room Owner 可以直接正常 End 這一場（不只 Abandon），之後在 Lobby 重新指派 DM 開新場；這是結束那一場，不是接管。Human DM 的 Session 不適用：Owner 仍只能 Abandon。
...
這是**同一隻 Mira**：同一 Build chain、同一 Current State、同一 Inventory、同一 HP / Prepared / Resources。
...
- 其他人可以留在 Room，但只有本場 DM Controller 能 End Combat、End Session、Request Check、裁定（唯一例外：本場 DM 是 AI 時，Owner 可正常 End Session，見下）。
- DM 重新連線即恢復（斷線重連不算更換 Controller，不違反「Session 中 DM Controller 固定」）。
- DM 確定不回來時，Session 就擱著；Owner 可把既有 Session 標為 `abandoned`，下一次重新指定 DM 並開新 Session。這不是 Owner 接管同一 Session。若那場的 DM 是 AI，Owner 也可以正常 End（見「Session End」）。
...
**聊天串跨 Session 連續。** 新 Session 的聊天串往上翻到本場 Start 後，可以繼續載入同一 Campaign 更早 Session 的可見事件，中間以 Session 分隔線標示（status、時間、DM 是真人或 AI）。舊 Session 唯讀；可見性仍由 Server 以 Seat 為單位過濾，DM 層跟著該場的 DM Seat——目前坐 DM Seat 的真人能回看 AI DM 留下的 DM 層紀錄，Player 看不到。這只是讀取入口，不是 Timeline／Snapshot；那些屬後續 Phase。
...
Web 首頁**沒有 global Character Workshop**。要創角、看角色、匯入角色，先進某個 Room。
...
- Create Draft 從建立當下就屬於該 Room。
- Import 一定指定目前 target Room。
- Character Confirm 後留在同一 Room。
- 不提供跨整台 Server 的 global Character list。
- 不做 `My Characters / Other Characters` ownership分類。
...
- 不含 Room id / Room code。
- 不含 Campaign / Seat / Session identity。
- 不含 Room Password / Owner Key / DM Key / AI Token。
- Web Import 在 target Room context建立**新的 Character identity**。
- Standalone Import建立本機新的 Character identity。
- 不做 merge。

M03 出貨時 schema 為 `unstable`；**P2 起鎖成 Character JSON v1**。P2 importer仍接受最後 M03 `unstable`格式並 normalize；從 v1 起新的 Adventure Table 版本必須持續能 import v1，後續變更不得任意改既有欄位語意。
...
- DM 可看完整 enemy current / max HP、AC、hidden resources、recharge 等戰鬥資料。
- Player / AI Player 預設**不收到敵人精確 HP、AC、hidden resources、recharge 或其他未公開資料**。
- Player 可看到公開名稱、公開 Condition、明顯 outcome，以及可選的模糊傷勢資訊，例如 `Wounded` / `Badly Wounded`。
- 這個差異必須由 Server projection / DTO 直接隔離，不能先把完整數值送到 client 再靠 UI 隱藏。
...
Adventure 是起始世界模板／可附加的世界資料包，不是 script。第一版**不另外建立 World Setting / Lorebook 類型**：同一種 Adventure Definition 可以是一個完整冒險模組，也可以只是世界設定、城市／勢力資料、幾個 NPC／地點，或其混合。

可包含：Sections、Scenes、NPCs、Monster refs、Items、Secrets、DM Notes、Suggested Checks、Maps。

上述內容全部 optional；Adventure 不需要有 Quest、固定流程或「開場→結局」結構。Campaign 也**不需要 Adventure 才能跑**：Human / AI DM 可以從空 Campaign 直接即興，靠 Exploration、Quick Add、Check、Quick Combat 與 Runtime write-back 讓世界逐漸長出來。

同一 Campaign 可同時 Attach **零到多個 Adventure**，沒有唯一 Active Adventure。Attach 只增加可查閱的世界／冒險來源；同名 NPC / Scene 不自動 merge，不自動猜是不是同一 entity。
...
Adventure Definition
= 原始模板
= gameplay / Runtime / AI world write 不修改
...
例如 Adventure：Innkeeper Neutral；Campaign：Innkeeper Hostile。Gameplay、Runtime 與 AI world write 不改 Adventure 本體；Room owner / DM 仍可手動做 authoring 修正，例如改 typo 或補內容。Campaign 只能 Attach 已 finalized 的 Adventure；手動建立的 Adventure 可直接 finalize，不必經 Importer。

Adventure Definition 是 DM 素材。Human Player / AI Player 第一版一律不可直接瀏覽 Adventure Definition，不論 entry 是否標 secret；玩家看到的世界只來自 DM 敘事、Stage、Runtime 公開內容與自己角色的 knowledge。
...
Campaign Runtime 另可保存 **optional Current Scene + optional Current Situation**，供跨 Session 接續與 AI context 使用。兩者都不是必填：Human DM 不按 Change Scene、只靠敘事也能繼續跑；Party 分頭行動仍不建立 per-character scene position。

P6 階段的 Map / Scene image 只作世界／Main Stage 素材，不提供 token coordinates、Wall / Door geometry、movement、range、AoE 或 LOS；這些 precise Tactical 能力仍屬 P5。
...
如果說「門被撞壞」要更新對應 world state；說「找到銀鑰匙」要建立／移轉 Item；即興的新 NPC / Scene / Fact 後續重要時要持久化。

Current AI DM 可以在既有 Role 權限內直接建立／修改 Campaign Runtime，不要求每筆變化等 Human approval。`needs_review=true` 只用於高影響、歧義或來源衝突，而且是提醒，不阻塞跑團。

若同一個高階 world action 同時宣告 canonical world change 與 definitive narration，必須先確定 state mutation 可成功，再發布「事情已經發生」的敘事；不能讓玩家看到「門已壞」但 Server truth 仍是未壞。P6 自己管理的 world write + narration 應以同一高階操作原子處理；既有 Roll / Combat 仍先由 P3/P4 canonical service resolve，再依 Server 結果敘事，不提前重造 P7 通用 GameTransaction。
...
Room owner / DM 在沒有 active Session 時也可整理 Runtime（NPC / Fact / Quest / Override / Current Situation），用於開場前準備與跨 Session 收尾；AI DM 只在 active Session 內寫入。
...
- Web Character 由一個 Room workspace管理。
- 未被 Campaign / Session history reference的 Character可依既有流程 Archive後永久 Delete。
- 已被 Campaign / Session history reference → Archive，不個別 Hard Delete。
- Character Version 永不因一般 Character archive被刪。
- Builder Draft 未 Confirm可 Cancel / Hard Delete。
...
- AI DM 主持時的任何還原功能
- Web global Character Workshop
- 同一 Character instance 跨 Room 共用
- Campaign-specific CharacterState copy
- Standalone 假 Room / hidden multiplayer schema作為產品需求
...
2026-09-19 P6 額外拍板：Adventure 同時承擔冒險模組與世界／Lore 資料包，不另建 Setting 類型；Campaign 可零到多個 Adventure；沒有 Adventure 仍可直接跑；Adventure 對 Human Player / AI Player 不可直接讀；Attach 需 finalized，finalized 後 Room owner / DM 仍可做 authoring 修正，手動 Adventure 可直接 finalize；Current Scene／Situation optional；Room owner / DM 可在 Session 外整理 Runtime，AI DM 只在 active Session 內寫入；AI DM 可直接維護 Runtime，`needs_review` 不阻塞；P6 圖片／地圖只作 Stage 素材、不偷跑 Tactical geometry；重要 world mutation 與 definitive narration 不得彼此失真。
